      ...
      child: Column(
        ...
        children: [
          ...
          _ShortcutTile(
            ...
            label: 'Note du jour',
            sublabel: 'Comportement + date libre',
            color: Colors.purpleAccent,
            onTap: () {
              onClose();
              _showQuickDayNote(context, fp);
            },
          ),
          _ShortcutTile(
  ...
  showModalBottomSheet(
    ...
    builder: (ctx) => StatefulBuilder(
      builder: (ctx, setS) => DraggableScrollableSheet(
        ...
        maxChildSize: 0.92,
        minChildSize: 0.5,
        builder: (_, scroll) => _bsContainer(
          child: ListView(
            controller: scroll,
            padding: const EdgeInsets.all(20),
            children: [
              _bsHandle(),
              _bsTitle('⭐ Bonus Rapide', Colors.greenAccent),
              const SizedBox(height: 16),
              const Text('Enfant', style: _labelSt),
              const SizedBox(height: 8),
              _childSelector(fp.children, selectedId,
                  (id) => setS(() => selectedId = id)),
              const SizedBox(height: 16),
              const Text('Raison', style: _labelSt),
              const SizedBox(height: 8),
              Wrap(
                spacing: 8,
                runSpacing: 8,
              ...
              if (selectedPreset == null) ...[
                const SizedBox(height: 16),
                const Text('Points personnalisés', style: _labelSt),
                const SizedBox(height: 8),
                _PointStepper(
                  value: customPoints,
                  color: Colors.greenAccent,
              ...
              _bsButton(
                ...
                onTap: () {
                  ...
                  Navigator.pop(ctx);
                  _snack(context,
                      '✅ +$pts pts attribués à ${child.name}', Colors.green);
                },
              ),
              const SizedBox(height: 24),
            ],
      ...
      builder: (ctx, setS) => DraggableScrollableSheet(
        ...
        builder: (_, scroll) => _bsContainer(
          child: ListView(
            ...
            children: [
              _bsHandle(),
              _bsTitle('⚡ Pénalité Rapide', Colors.redAccent),
              const SizedBox(height: 16),
              const Text('Enfant', style: _labelSt),
              const SizedBox(height: 8),
              _childSelector(fp.children, selectedId,
              ...
              _bsButton(
                ...
                onTap: () {
                  ...
                  Navigator.pop(ctx);
                  _snack(context,
                      '⚡ -$pts pts appliqués à ${child.name}', Colors.red);
                },
              ),
              const SizedBox(height: 24),
            ],
        ...
        return DraggableScrollableSheet(
          ...
          builder: (_, scroll) => _bsContainer(
            child: ListView(
              ...
              children: [
                _bsHandle(),
                _bsTitle('🧠 Note du Jour', Colors.purpleAccent),
                const SizedBox(height: 16),
                GestureDetector(
                  onTap: () async {
                ...
                _childSelector(fp.children, selectedId,
                    (id) => setS(() => selectedId = id)),
                const SizedBox(height: 16),
                const Text('Critère', style: _labelSt),
                const SizedBox(height: 8),
                Wrap(
                  spacing: 8,
                  runSpacing: 8,
                ...
                const Text('Note /20', style: _labelSt),
                const SizedBox(height: 10),
                Row(
                ...
                Wrap(
                  ...
                  spacing: 6,
                  children: [5, 8, 10, 12, 14, 16, 18, 20].map((v) {
                    final isSel = noteValue == v;
                    return GestureDetector(
                ...
                _bsButton(
                  ...
                  onTap: () {
                    ...
                    _snack(
                      context,
                      '🧠 Note ${_fmtDate(noteDate)} — ${child.name}: $noteValue/$maxValue',
                      Colors.purple,
                    );
                  },
                ),
      ...
      builder: (ctx, setS) => DraggableScrollableSheet(
        ...
        builder: (_, scroll) => _bsContainer(
          child: ListView(
            ...
            children: [
              _bsHandle(),
              _bsTitle('📏 Punition Rapide', Colors.orangeAccent),
              const SizedBox(height: 16),
              const Text('Enfant', style: _labelSt),
              const SizedBox(height: 8),
              _childSelector(fp.children, selectedId,
                  (id) => setS(() => selectedId = id)),
              const SizedBox(height: 16),
              const Text('Motif', style: _labelSt),
              const SizedBox(height: 8),
              Wrap(
                spacing: 8,
                runSpacing: 8,
              ...
              const SizedBox(height: 16),
              const Text('Nombre de lignes', style: _labelSt),
              const SizedBox(height: 8),
              Wrap(
                alignment: WrapAlignment.center,
                spacing: 8,
...
// ══════════════════════════════════════════════════════════════
//  IMMUNITÉ RAPIDE  ← CORRIGÉ : ordre (childId, reason, lines)
// ══════════════════════════════════════════════════════════════
      ...
      builder: (ctx, setS) => DraggableScrollableSheet(
        ...
        builder: (_, scroll) => _bsContainer(
          child: ListView(
            ...
            children: [
              _bsHandle(),
              _bsTitle('🛡️ Immunité Rapide', Colors.amberAccent),
              const SizedBox(height: 16),
              const Text('Enfant', style: _labelSt),
              const SizedBox(height: 8),
              _childSelector(fp.children, selectedId,
              ...
              const SizedBox(height: 16),
              const Text("Nombre de lignes d'immunité", style: _labelSt),
              const SizedBox(height: 8),
              Wrap(
                alignment: WrapAlignment.center,
                spacing: 8,
...
// ══════════════════════════════════════════════════════════════
//  BONUS TEMPS ÉCRAN  ← CORRIGÉ : 3 paramètres
// ══════════════════════════════════════════════════════════════
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              _bsHandle(),
              const SizedBox(height: 16),
              _bsTitle("⏱️ Bonus Temps d'Écran", Colors.lightBlueAccent),
              const SizedBox(height: 16),
              const Text('Enfant', style: _labelSt),
              const SizedBox(height: 8),
              _childSelector(fp.children, selectedId,
                ...
                children: [15, 30, 45, 60, 90].map((min) {
                  return GestureDetector(
                    onTap: () {
                      ...
                      // ✅ CORRIGÉ : 3 paramètres (childId, minutes, reason)
                      fp.addScreenTimeBonus(
                          selectedId, min, 'Bonus écran rapide');
                      Navigator.pop(ctx);
                      _snack(context,
    ...
    builder: (ctx) => DraggableScrollableSheet(
      ...
      builder: (_, scroll) => _bsContainer(
        child: ListView(
          ...
          children: [
            _bsHandle(),
            _bsTitle('📊 Bilan du ${_fmtDate(today)}', Colors.tealAccent),
            const SizedBox(height: 16),
            if (fp.children.isEmpty)
              ...
              ...fp.children.map((child) {
                ...
                final pointsToday = todayEntries.fold<int>(
                    0, (s, h) => s + h.points);
                final bonuses =
                      ...
                      children: [
                        Row(children: [
                          ...
                          Container(
                            ...
                            decoration: BoxDecoration(
                              color: ptColor.withOpacity(0.15),
                              borderRadius: BorderRadius.circular(10),
                            ),
                            child: Text(
                        ...
                        Wrap(
                          ...
                          runSpacing: 6,
                          children: [
                            _chip('✅ $bonuses bonus', Colors.greenAccent),
                            _chip('⚡ $penalties pénalités',
                                Colors.redAccent),
                            if (avgNote != null)
                              _chip('🧠 ${avgNote.round()}/20',
                                  Colors.purpleAccent),
                          ],
                        ),
...
Widget _bsContainer({required Widget child}) => Container(
...
Widget _bsTitle(String text, Color color) => Center(
      child: Padding(
...
const _labelSt = TextStyle(color: Colors.white70, fontSize: 14);
