...
class _HandshakeAnimationState extends State<_HandshakeAnimation> with TickerProviderStateMixin {
  late AnimationController _mainCtrl;
  late Animation<double> _leftHand;
  late Animation<double> _rightHand;
  ...
  void initState() {
    ...
    _rightHand = Tween<double>(begin: 120.0, end: 0.0).animate(
      CurvedAnimation(parent: _mainCtrl, curve: const Interval(0.0, 0.35, curve: Curves.easeOutCubic)));

    // Secousse poignée de main
          ...
          Positioned(
            ...
            child: FadeTransition(
              ...
              child: Column(mainAxisSize: MainAxisSize.min, children: [
                Text(widget.message, style: const TextStyle(color: Colors.amber, fontSize: 22, fontWeight: FontWeight.w900, letterSpacing: 2, shadows: [Shadow(color: Colors.orangeAccent, blurRadius: 15)])),
              ]),
            ),
          ),
        ]);
...
class _SparkDot {
  ...
  final Color color;
  _SparkDot({required this.angle, required this.speed, required this.size, required this.color});
}
...
// ═══════════════════════════════════════════════════════════
//  CONFETTIS CÉLÉBRATION (vente complétée)
// ═══════════════════════════════════════════════════════════
    ...
    return AnimatedBuilder(
      ...
      builder: (context, _) {
        ...
        return Stack(alignment: Alignment.center, children: [
          ...
          if (t > 0.15 && t < 0.85)
            ScaleTransition(
              scale: Tween<double>(begin: 0.0, end: 1.0).animate(CurvedAnimation(parent: _ctrl, curve: const Interval(0.15, 0.45, curve: Curves.elasticOut))),
...
class _ConfettiRect {
  ...
  final Color color;
  _ConfettiRect({required this.x, required this.speed, required this.size, required this.color, required this.rotSpeed, required this.wobble});
}
...
class _ConfettiRectPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    for (final c in confetti) {
      ...
      final paint = Paint()..color = c.color.withOpacity(opacity);
      canvas.drawRRect(
        RRect.fromRectAndRadius(Rect.fromCenter(center: Offset.zero, width: c.size, height: c.size * 0.5), const Radius.circular(1)),
        paint,
      );
      canvas.restore();
    }
...
class _TradeScreenState extends State<TradeScreen> with SingleTickerProviderStateMixin {
  ...
  Widget build(BuildContext context) {
    return Consumer<FamilyProvider>(
      builder: (context, provider, _) {
        ...
        if (child == null) {
          return Scaffold(backgroundColor: const Color(0xFF0a0a2a), body: const Center(child: Text('Enfant non trouvé', style: TextStyle(color: Colors.white))));
        }
          ...
          appBar: AppBar(
            title: Row(mainAxisSize: MainAxisSize.min, children: [
              TweenAnimationBuilder<double>(
                tween: Tween<double>(begin: 0, end: 1), duration: const Duration(milliseconds: 800), curve: Curves.elasticOut,
            ...
            bottom: TabBar(
              controller: _tabController, indicatorColor: Colors.amber, labelColor: Colors.amber, unselectedLabelColor: Colors.white54,
              tabs: [
      ...
      child: Row(children: [
        ...
        TweenAnimationBuilder<double>(
          ...
          builder: (context, val, _) => Container(
            ...
            child: Text('${val.round()}', style: TextStyle(color: available > 0 ? const Color(0xFF00E676) : Colors.white38, fontWeight: FontWeight.w900, fontSize: 20)),
          ),
        ),
      ]),
    );
  ...
  Widget _buildMarketTab(FamilyProvider provider, ChildModel child, int available, List<TradeModel> pendingForMe) {
    ...
    return ListView(padding: const EdgeInsets.all(16), children: [
      if (pendingForMe.isNotEmpty) ...[
        _buildSectionTitle('📩 Offres reçues', Colors.amber), const SizedBox(height: 8),
        ...pendingForMe.map((trade) => _buildPendingOfferCard(provider, trade, isReceived: true)),
        const SizedBox(height: 20),
      ],
      if (myPendingSales.isNotEmpty) ...[
        _buildSectionTitle('📤 Mes ventes en attente', const Color(0xFF00E676)), const SizedBox(height: 8),
        ...
        if (isReceived)
          Row(children: [
            Expanded(child: TvFocusWrapper(
              onTap: () => provider.rejectTrade(trade.id),
              child: ElevatedButton.icon(onPressed: () => provider.rejectTrade(trade.id),
          ...
          SizedBox(width: double.infinity, child: TvFocusWrapper(
            onTap: () => provider.cancelTrade(trade.id),
            child: ElevatedButton.icon(onPressed: () => provider.cancelTrade(trade.id),
  ...
  Widget _buildActiveTab(FamilyProvider provider, ChildModel child, List<TradeModel> activeTrades) {
    ...
    return ListView(padding: const EdgeInsets.all(16), children: inProgress.map((trade) => _buildActiveTradeCard(provider, trade)).toList());
  }
    ...
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        if (trade.isAccepted && !isSeller)
          SizedBox(width: double.infinity, child: TvFocusWrapper(
            onTap: () async { await showHandshakeAnimation(context, 'SERVICE RENDU !'); if (mounted) provider.markServiceDone(trade.id); },
            child: ElevatedButton.icon(
              onPressed: () async { await showHandshakeAnimation(context, 'SERVICE RENDU !'); if (mounted) provider.markServiceDone(trade.id); },
        ...
        Center(child: TextButton(onPressed: () => provider.cancelTrade(trade.id), child: const Text('Annuler', style: TextStyle(color: Colors.red, fontSize: 12)))),
      ]),
    );
  }
      ...
      return Container(
        ...
        child: Row(children: [
          ...
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            Text('${trade.immunityLines} ligne${trade.immunityLines > 1 ? 's' : ''} • ${trade.serviceDescription}', style: TextStyle(color: Colors.white.withOpacity(0.5), fontSize: 12), maxLines: 1, overflow: TextOverflow.ellipsis),
  ...
  void _showCreateSaleDialog(BuildContext context, FamilyProvider provider, ChildModel seller, int maxLines) {
    ...
    if (otherChildren.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: const Text('Il faut au moins 2 enfants pour une vente'), backgroundColor: Colors.red, behavior: SnackBarBehavior.floating, shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12))));
      return;
    }
    showDialog(context: context, builder: (ctx) {
      String? selectedBuyerId; int lines = 1; String service = '';
      return StatefulBuilder(builder: (ctx, setDialogState) {
        final bool canSubmit = selectedBuyerId != null && service.trim().isNotEmpty;
        return AlertDialog(
          backgroundColor: const Color(0xFF1a1a4a), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(20)),
          title: Row(children: [
            TweenAnimationBuilder<double>(tween: Tween<double>(begin: 0, end: 1), duration: const Duration(milliseconds: 600), curve: Curves.elasticOut,
              builder: (context, val, child) => Transform.scale(scale: val, child: child),
          ...
          content: SingleChildScrollView(child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              return TvFocusWrapper(
                ...
                child: GestureDetector(onTap: () => setDialogState(() => selectedBuyerId = c.id),
                  child: AnimatedContainer(duration: const Duration(milliseconds: 200),
                    padding: const EdgeInsets.symmetric(horizontal: 14, vertical: 8),
                    decoration: BoxDecoration(color: isSelected ? const Color(0xFF00E676).withOpacity(0.2) : Colors.white.withOpacity(0.06), borderRadius: BorderRadius.circular(12),
                      border: Border.all(color: isSelected ? const Color(0xFF00E676) : Colors.white24, width: isSelected ? 2 : 1)),
                    child: Row(mainAxisSize: MainAxisSize.min, children: [
                      Text(c.avatar.isNotEmpty ? c.avatar : '👤', style: const TextStyle(fontSize: 16)), const SizedBox(width: 6),
            ...
            Row(mainAxisAlignment: MainAxisAlignment.center, children: [
              IconButton(onPressed: lines > 1 ? () => setDialogState(() => lines--) : null, icon: Icon(Icons.remove_circle_rounded, color: lines > 1 ? Colors.red : Colors.grey, size: 32)),
              Container(padding: const EdgeInsets.symmetric(horizontal: 24, vertical: 8), decoration: BoxDecoration(color: Colors.white.withOpacity(0.1), borderRadius: BorderRadius.circular(12)),
                child: Text('$lines', style: const TextStyle(color: Color(0xFF00E676), fontSize: 28, fontWeight: FontWeight.w900))),
              IconButton(onPressed: lines < maxLines ? () => setDialogState(() => lines++) : null, icon: Icon(Icons.add_circle_rounded, color: lines < maxLines ? const Color(0xFF00E676) : Colors.grey, size: 32)),
            ]),
            ...
            const SizedBox(height: 8),
            TextField(onChanged: (val) => setDialogState(() => service = val), style: const TextStyle(color: Colors.white), maxLines: 2,
              decoration: InputDecoration(hintText: 'Ex: Ranger ma chambre, faire la vaisselle...', hintStyle: const TextStyle(color: Colors.white30), filled: true, fillColor: Colors.white.withOpacity(0.08),
          ...
          actions: [
            ...
            TvFocusWrapper(
              onTap: canSubmit ? () async {
                ...
                if (mounted) {
                  await showHandshakeAnimation(context, 'VENTE PROPOSÉE !');
                }
              } : null,
              child: ElevatedButton.icon(
      ...
      return AlertDialog(
        ...
        content: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.start, children: [
          Container(width: double.infinity, padding: const EdgeInsets.all(12),
            ...
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Text('${seller?.name ?? "?"} vend ${trade.immunityLines} ligne${trade.immunityLines > 1 ? 's' : ''} à ${buyer?.name ?? "?"}', style: const TextStyle(color: Colors.white, fontWeight: FontWeight.w600)),
              const SizedBox(height: 6),
          ...
          const SizedBox(height: 8),
          TextField(onChanged: (val) => note = val, style: const TextStyle(color: Colors.white),
            decoration: InputDecoration(hintText: 'Commentaire...', hintStyle: const TextStyle(color: Colors.white30), filled: true, fillColor: Colors.white.withOpacity(0.08),
              border: OutlineInputBorder(borderRadius: BorderRadius.circular(12), borderSide: BorderSide.none))),
        ]),
        actions: [
          TextButton(onPressed: () => Navigator.pop(ctx), child: const Text('Annuler', style: TextStyle(color: Colors.white54))),
  ...
  Widget _buildBadgeCount(int count) {
    return Container(padding: const EdgeInsets.symmetric(horizontal: 7, vertical: 2), decoration: BoxDecoration(color: Colors.red, borderRadius: BorderRadius.circular(10)),
