...
class _HandshakeAnimationState extends State<_HandshakeAnimation>
  ...
  void initState() {
    ...
    _leftHand = Tween<double>(begin: -120.0, end: 0.0).animate(
        CurvedAnimation(parent: _mainCtrl, curve: const Interval(0.0, 0.35, curve: Curves.easeOutCubic)));
    ...
    ]).animate(CurvedAnimation(parent: _mainCtrl, curve: const Interval(0.35, 0.7, curve: Curves.easeInOut)));
    _textFade = Tween<double>(begin: 0.0, end: 1.0).animate(
        CurvedAnimation(parent: _mainCtrl, curve: const Interval(0.5, 0.75, curve: Curves.easeIn)));
    _sparkCtrl = AnimationController(vsync: this, duration: const Duration(milliseconds: 1500));
    ...
    Future.delayed(const Duration(milliseconds: 700), () { if (mounted) _sparkCtrl.forward(); });
  }
          ...
          Transform.translate(
            ...
            child: Row(mainAxisSize: MainAxisSize.min, children: [
              Transform.translate(offset: Offset(_leftHand.value, 0),
                child: const Text('\u{1F91D}', style: TextStyle(fontSize: 72))),
            ]),
          ),
          Positioned(
            ...
            child: FadeTransition(opacity: _textFade,
              child: Text(widget.message, style: const TextStyle(
                color: Colors.amber, fontSize: 22, fontWeight: FontWeight.w900,
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (final s in sparks) {
      ...
      canvas.drawCircle(Offset(cx + cos(s.angle) * dist, cy + sin(s.angle) * dist), s.size,
        Paint()..color = s.color.withOpacity((1.0 - t).clamp(0.0, 1.0) * 0.8)
...
class _TradeCelebrationState extends State<_TradeCelebration>
  ...
  void initState() {
    ...
    _confetti = List.generate(40, (i) => _ConfettiRect(
      x: _rng.nextDouble(), speed: 120 + _rng.nextDouble() * 250,
        ...
        if (t > 0.15 && t < 0.85)
          ScaleTransition(
            ...
            child: const Column(mainAxisSize: MainAxisSize.min, children: [
              Text('\u{1F389}', style: TextStyle(fontSize: 52)),
              SizedBox(height: 8),
              Text('VENTE VALID\u00C9E !', style: TextStyle(
                color: Colors.greenAccent, fontSize: 24, fontWeight: FontWeight.w900, letterSpacing: 3)),
              SizedBox(height: 4),
  ...
  void paint(Canvas canvas, Size size) {
    for (final c in confetti) {
      ...
      canvas.drawRRect(RRect.fromRectAndRadius(
        Rect.fromCenter(center: Offset.zero, width: c.size, height: c.size * 0.5), const Radius.circular(1)),
...
class _TradeScreenState extends State<TradeScreen> with SingleTickerProviderStateMixin {
  ...
  Widget build(BuildContext context) {
    return Consumer<FamilyProvider>(builder: (context, provider, _) {
      ...
      if (child == null) {
        return Scaffold(backgroundColor: const Color(0xFF0a0a2a),
      ...
      return Scaffold(
        ...
        appBar: AppBar(
          title: Row(mainAxisSize: MainAxisSize.min, children: [
            ...
            Text('Vente d\'immunit\u00E9s', style: TextStyle(fontSize: isTV ? 24 : 18)),
          ]),
          backgroundColor: Colors.transparent, elevation: 0,
          actions: [
            if (availableImmunity > 0)
              Padding(
                padding: EdgeInsets.only(right: isTV ? 24 : 8),
                child: TvFocusWrapper(
                  onTap: () => _showCreateSaleDialog(context, provider, child, availableImmunity),
                  child: Container(
                    padding: EdgeInsets.symmetric(horizontal: isTV ? 20 : 14, vertical: isTV ? 10 : 6),
                    ...
                    child: Row(mainAxisSize: MainAxisSize.min, children: [
                      Icon(Icons.sell_rounded, color: Colors.white, size: isTV ? 22 : 18),
                      const SizedBox(width: 6),
                      Text('Vendre', style: TextStyle(
          ...
          bottom: TabBar(
            ...
            tabs: [
              Tab(child: Row(mainAxisSize: MainAxisSize.min, children: [
                Text('March\u00E9', style: TextStyle(fontSize: isTV ? 18 : 14)),
                if (pendingForMe.isNotEmpty) ...[const SizedBox(width: 6), _buildBadgeCount(pendingForMe.length)],
              ])),
              Tab(child: Row(mainAxisSize: MainAxisSize.min, children: [
                Text('En cours', style: TextStyle(fontSize: isTV ? 18 : 14)),
                if (activeTrades.isNotEmpty) ...[const SizedBox(width: 6), _buildBadgeCount(activeTrades.length)],
              ])),
              Tab(child: Text('Historique', style: TextStyle(fontSize: isTV ? 18 : 14))),
            ],
          ),
        ),
  ...
  Widget _buildImmunityBanner(ChildModel child, int available) {
    return Container(
      margin: EdgeInsets.fromLTRB(isTV ? 24 : 16, 12, isTV ? 24 : 16, 4),
      padding: EdgeInsets.symmetric(horizontal: isTV ? 20 : 16, vertical: isTV ? 16 : 12),
      decoration: BoxDecoration(
        gradient: LinearGradient(colors: [
          const Color(0xFF00E676).withOpacity(0.12), const Color(0xFF00E676).withOpacity(0.04)]),
        borderRadius: BorderRadius.circular(16),
        border: Border.all(color: const Color(0xFF00E676).withOpacity(0.3)),
      ),
      child: Row(children: [
        Container(width: isTV ? 52 : 44, height: isTV ? 52 : 44,
          decoration: BoxDecoration(color: const Color(0xFF00E676).withOpacity(0.15), borderRadius: BorderRadius.circular(12)),
          child: Icon(Icons.shield_rounded, color: const Color(0xFF00E676), size: isTV ? 28 : 24)),
        SizedBox(width: isTV ? 16 : 14),
        Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Text(child.name, style: TextStyle(color: Colors.white, fontWeight: FontWeight.w700, fontSize: isTV ? 20 : 15)),
          Text(available > 0
            ? '$available ligne${available > 1 ? 's' : ''} d\'immunit\u00E9 disponible${available > 1 ? 's' : ''}'
            : 'Aucune immunit\u00E9 \u00E0 vendre',
            style: TextStyle(color: available > 0 ? const Color(0xFF00E676) : Colors.white38,
              fontSize: isTV ? 16 : 12, fontWeight: FontWeight.w500)),
        ])),
        Container(
          padding: EdgeInsets.symmetric(horizontal: isTV ? 16 : 14, vertical: isTV ? 10 : 8),
          decoration: BoxDecoration(
            color: available > 0 ? const Color(0xFF00E676).withOpacity(0.2) : Colors.white.withOpacity(0.05),
            borderRadius: BorderRadius.circular(12),
            border: Border.all(color: available > 0 ? const Color(0xFF00E676).withOpacity(0.5) : Colors.white.withOpacity(0.1))),
          child: Text('$available', style: TextStyle(
            color: available > 0 ? const Color(0xFF00E676) : Colors.white38,
            fontWeight: FontWeight.w900, fontSize: isTV ? 24 : 20)),
        ),
      ]),
    );
  }
  ...
  Widget _buildMarketTab(FamilyProvider provider, ChildModel child, int available, List<TradeModel> pendingForMe) {
    ...
    if (pendingForMe.isEmpty && myPendingSales.isEmpty) {
      return Center(child: Column(mainAxisSize: MainAxisSize.min, children: [
        Icon(Icons.storefront_rounded, size: isTV ? 80 : 70, color: Colors.white.withOpacity(0.1)),
        const SizedBox(height: 16),
        Text('Aucune offre en cours', style: TextStyle(color: Colors.white.withOpacity(0.4), fontSize: isTV ? 22 : 16)),
        if (available > 0) ...[const SizedBox(height: 8),
          Text('Appuyez sur "Vendre" pour proposer une vente',
            style: TextStyle(color: Colors.white.withOpacity(0.25), fontSize: isTV ? 16 : 13))],
      ]));
    }
    return ListView(padding: EdgeInsets.all(isTV ? 24 : 16), children: [
      if (pendingForMe.isNotEmpty) ...[
        _buildSectionTitle('\u{1F4E9} Offres re\u00E7ues', Colors.amber),
        const SizedBox(height: 8),
        ...pendingForMe.map((t) => _buildPendingOfferCard(provider, t, isReceived: true)),
        const SizedBox(height: 20),
      ],
      if (myPendingSales.isNotEmpty) ...[
        _buildSectionTitle('\u{1F4E4} Mes ventes en attente', const Color(0xFF00E676)),
        const SizedBox(height: 8),
  ...
  Widget _buildPendingOfferCard(FamilyProvider provider, TradeModel trade, {required bool isReceived}) {
    ...
    return Container(
      margin: EdgeInsets.only(bottom: isTV ? 16 : 12),
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(children: [
          Container(width: isTV ? 48 : 40, height: isTV ? 48 : 40,
            decoration: BoxDecoration(color: Colors.amber.withOpacity(0.15), borderRadius: BorderRadius.circular(10)),
            child: Center(child: Text(otherChild?.avatar.isNotEmpty == true ? otherChild!.avatar : '\u{1F464}',
              style: TextStyle(fontSize: isTV ? 24 : 20)))),
          SizedBox(width: isTV ? 14 : 12),
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Text(isReceived ? '${seller?.name ?? "?"} te vend' : 'Vente \u00E0 ${buyer?.name ?? "?"}',
              style: TextStyle(color: Colors.white, fontWeight: FontWeight.w700, fontSize: isTV ? 18 : 14)),
            Text('${trade.immunityLines} ligne${trade.immunityLines > 1 ? 's' : ''} d\'immunit\u00E9',
              style: TextStyle(color: const Color(0xFF00E676), fontSize: isTV ? 16 : 13, fontWeight: FontWeight.w600)),
          ])),
          Container(
            padding: EdgeInsets.symmetric(horizontal: isTV ? 12 : 10, vertical: isTV ? 8 : 6),
            decoration: BoxDecoration(color: Colors.amber.withOpacity(0.15), borderRadius: BorderRadius.circular(10),
              border: Border.all(color: Colors.amber.withOpacity(0.4))),
            child: Row(mainAxisSize: MainAxisSize.min, children: [
              Icon(Icons.shield_rounded, color: Colors.amber, size: isTV ? 20 : 16),
              const SizedBox(width: 4),
              Text('${trade.immunityLines}', style: TextStyle(
                color: Colors.amber, fontWeight: FontWeight.w900, fontSize: isTV ? 20 : 16)),
            ]),
          ),
        ]),
        SizedBox(height: isTV ? 14 : 12),
        Container(
          width: double.infinity, padding: EdgeInsets.all(isTV ? 14 : 12),
          decoration: BoxDecoration(color: Colors.white.withOpacity(0.04), borderRadius: BorderRadius.circular(10)),
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Text('\u{1F4BC} Service demand\u00E9 :', style: TextStyle(color: Colors.white.withOpacity(0.5), fontSize: isTV ? 14 : 11)),
            const SizedBox(height: 4),
            Text(trade.serviceDescription, style: TextStyle(color: Colors.white, fontSize: isTV ? 18 : 14)),
          ]),
        ),
        SizedBox(height: isTV ? 14 : 12),
        if (isReceived)
          Row(children: [
            Expanded(child: TvFocusWrapper(
              ...
              child: Container(
                ...
                child: Row(mainAxisAlignment: MainAxisAlignment.center, children: [
                  Icon(Icons.close_rounded, size: isTV ? 20 : 18, color: Colors.red),
                  const SizedBox(width: 6),
                  Text('Refuser', style: TextStyle(color: Colors.red, fontWeight: FontWeight.bold, fontSize: isTV ? 16 : 14)),
                ]),
              ),
            )),
            ...
            Expanded(child: TvFocusWrapper(
              ...
              child: Container(
                ...
                child: Row(mainAxisAlignment: MainAxisAlignment.center, children: [
                  Icon(Icons.check_rounded, size: isTV ? 20 : 18, color: const Color(0xFF00E676)),
                  const SizedBox(width: 6),
                  Text('Accepter', style: TextStyle(color: const Color(0xFF00E676), fontWeight: FontWeight.bold, fontSize: isTV ? 16 : 14)),
                ]),
              ),
            )),
          ])
          ...
          TvFocusWrapper(
            ...
            child: Container(
              ...
              child: Row(mainAxisAlignment: MainAxisAlignment.center, children: [
                Icon(Icons.cancel_outlined, size: isTV ? 20 : 18, color: Colors.red),
                const SizedBox(width: 6),
                Text('Annuler la vente', style: TextStyle(color: Colors.red, fontWeight: FontWeight.bold, fontSize: isTV ? 16 : 14)),
              ]),
            ),
          ),
      ]),
  ...
  Widget _buildActiveTab(FamilyProvider provider, ChildModel child, List<TradeModel> activeTrades) {
    ...
    if (inProgress.isEmpty) {
      return Center(child: Column(mainAxisSize: MainAxisSize.min, children: [
        Icon(Icons.hourglass_empty_rounded, size: isTV ? 80 : 70, color: Colors.white.withOpacity(0.1)),
        const SizedBox(height: 16),
        Text('Aucune vente en cours', style: TextStyle(color: Colors.white.withOpacity(0.4), fontSize: isTV ? 22 : 16)),
      ]));
    }
    return ListView(padding: EdgeInsets.all(isTV ? 24 : 16),
      children: inProgress.map((t) => _buildActiveTradeCard(provider, t)).toList());
  }
  ...
  Widget _buildActiveTradeCard(FamilyProvider provider, TradeModel trade) {
    ...
    if (trade.isAccepted) {
      statusColor = Colors.orange; statusText = 'En attente du service'; statusIcon = Icons.pending_actions_rounded;
    } else {
      statusColor = const Color(0xFF7C4DFF); statusText = 'Service rendu \u2013 validation parent'; statusIcon = Icons.verified_rounded;
    }
    ...
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Container(
          padding: EdgeInsets.symmetric(horizontal: isTV ? 12 : 10, vertical: isTV ? 6 : 5),
          decoration: BoxDecoration(color: statusColor.withOpacity(0.12), borderRadius: BorderRadius.circular(8)),
          child: Row(mainAxisSize: MainAxisSize.min, children: [
            Icon(statusIcon, color: statusColor, size: isTV ? 18 : 14),
            const SizedBox(width: 6),
            Text(statusText, style: TextStyle(color: statusColor, fontSize: isTV ? 16 : 12, fontWeight: FontWeight.w600)),
          ]),
        ),
        SizedBox(height: isTV ? 14 : 12),
        Row(children: [
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Text('${seller?.name ?? "?"} \u2192 ${buyer?.name ?? "?"}',
              style: TextStyle(color: Colors.white, fontWeight: FontWeight.w700, fontSize: isTV ? 20 : 15)),
            const SizedBox(height: 4),
            Text('${trade.immunityLines} ligne${trade.immunityLines > 1 ? 's' : ''} d\'immunit\u00E9',
              style: TextStyle(color: const Color(0xFF00E676), fontSize: isTV ? 16 : 13)),
          ])),
        ]),
        SizedBox(height: isTV ? 12 : 10),
        Container(
          width: double.infinity, padding: EdgeInsets.all(isTV ? 12 : 10),
          decoration: BoxDecoration(color: Colors.white.withOpacity(0.04), borderRadius: BorderRadius.circular(10)),
          child: Text('\u{1F4BC} ${trade.serviceDescription}',
        ...
        if (trade.isAccepted && !isSeller)
          TvFocusWrapper(
            ...
            child: Container(
              width: double.infinity,
              padding: EdgeInsets.symmetric(vertical: isTV ? 14 : 12),
              decoration: BoxDecoration(color: const Color(0xFF00E676).withOpacity(0.2), borderRadius: BorderRadius.circular(12),
                border: Border.all(color: const Color(0xFF00E676).withOpacity(0.4))),
              child: Row(mainAxisAlignment: MainAxisAlignment.center, children: [
                Icon(Icons.check_circle_rounded, size: isTV ? 22 : 18, color: const Color(0xFF00E676)),
                const SizedBox(width: 8),
                Text('J\'ai rendu le service', style: TextStyle(
                  color: const Color(0xFF00E676), fontWeight: FontWeight.bold, fontSize: isTV ? 18 : 14)),
              ]),
            ),
          ),
        if (trade.isServiceDone)
          TvFocusWrapper(
            ...
            child: Container(
              width: double.infinity,
              padding: EdgeInsets.symmetric(vertical: isTV ? 14 : 12),
              decoration: BoxDecoration(color: const Color(0xFF7C4DFF).withOpacity(0.2), borderRadius: BorderRadius.circular(12),
                border: Border.all(color: const Color(0xFF7C4DFF).withOpacity(0.4))),
              child: Row(mainAxisAlignment: MainAxisAlignment.center, children: [
                Icon(Icons.gavel_rounded, size: isTV ? 22 : 18, color: const Color(0xFF7C4DFF)),
                const SizedBox(width: 8),
                Text('Validation parent', style: TextStyle(
                  color: const Color(0xFF7C4DFF), fontWeight: FontWeight.bold, fontSize: isTV ? 18 : 14)),
              ]),
            ),
          ),
        ...
        Center(child: TvFocusWrapper(
          ...
          child: Padding(
            ...
            child: Text('Annuler', style: TextStyle(color: Colors.red, fontSize: isTV ? 16 : 12)),
          ),
        )),
      ]),
    );
  ...
  Widget _buildHistoryTab(FamilyProvider provider, ChildModel child, List<TradeModel> trades) {
    if (trades.isEmpty) {
      return Center(child: Column(mainAxisSize: MainAxisSize.min, children: [
        Icon(Icons.history_rounded, size: isTV ? 80 : 70, color: Colors.white.withOpacity(0.1)),
        const SizedBox(height: 16),
        Text('Aucune vente termin\u00E9e', style: TextStyle(color: Colors.white.withOpacity(0.4), fontSize: isTV ? 22 : 16)),
      ]));
    }
    return ListView(padding: EdgeInsets.all(isTV ? 24 : 16), children: trades.map((trade) {
      final seller = provider.getChild(trade.fromChildId);
      final buyer = provider.getChild(trade.toChildId);
      ...
      return Container(
        ...
        child: Row(children: [
          Container(width: isTV ? 48 : 40, height: isTV ? 48 : 40,
            ...
            child: Center(child: Text(trade.statusEmoji, style: TextStyle(fontSize: isTV ? 22 : 18)))),
          SizedBox(width: isTV ? 14 : 12),
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Text('${seller?.name ?? "?"} \u2192 ${buyer?.name ?? "?"}',
              style: TextStyle(color: Colors.white, fontWeight: FontWeight.w600, fontSize: isTV ? 18 : 13)),
            Text('${trade.immunityLines} ligne${trade.immunityLines > 1 ? 's' : ''} \u2022 ${trade.serviceDescription}',
              style: TextStyle(color: Colors.white.withOpacity(0.5), fontSize: isTV ? 16 : 12),
              maxLines: 1, overflow: TextOverflow.ellipsis),
            Text(dateStr, style: TextStyle(color: Colors.white.withOpacity(0.3), fontSize: isTV ? 14 : 11)),
          ])),
          Container(
            padding: EdgeInsets.symmetric(horizontal: isTV ? 10 : 8, vertical: isTV ? 6 : 4),
            decoration: BoxDecoration(color: statusColor.withOpacity(0.12), borderRadius: BorderRadius.circular(8)),
            child: Text(trade.statusLabel, style: TextStyle(
              color: statusColor, fontSize: isTV ? 14 : 11, fontWeight: FontWeight.w600)),
          ),
        ]),
      );
    }).toList());
  ...
  void _showCreateSaleDialog(BuildContext context, FamilyProvider provider, ChildModel seller, int maxLines) {
    ...
    showDialog(context: context, builder: (ctx) {
      ...
      return StatefulBuilder(builder: (ctx, setDialogState) {
        ...
        final bool canSubmit = selectedBuyerId != null && serviceCtrl.text.trim().isNotEmpty && parsedLines > 0 && parsedLines <= maxLines;
        ...
        return AlertDialog(
          ...
          title: Row(children: [
            Icon(Icons.sell_rounded, color: const Color(0xFF00E676), size: isTV ? 26 : 22),
            const SizedBox(width: 8),
            Text('Nouvelle vente', style: TextStyle(color: const Color(0xFF00E676), fontSize: isTV ? 24 : 18)),
          ]),
          content: SingleChildScrollView(child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.start, children: [
            Text('Vendre \u00E0 :', style: TextStyle(color: Colors.white70, fontSize: isTV ? 18 : 13)),
            const SizedBox(height: 8),
            Wrap(spacing: 8, runSpacing: 8, children: otherChildren.map((c) {
              final isSel = selectedBuyerId == c.id;
              return TvFocusWrapper(
                onTap: () => setDialogState(() => selectedBuyerId = c.id),
                child: Container(
                  padding: EdgeInsets.symmetric(horizontal: isTV ? 16 : 14, vertical: isTV ? 10 : 8),
                  decoration: BoxDecoration(
                    color: isSel ? const Color(0xFF00E676).withOpacity(0.2) : Colors.white.withOpacity(0.06),
                    borderRadius: BorderRadius.circular(12),
                    border: Border.all(color: isSel ? const Color(0xFF00E676) : Colors.white24, width: isSel ? 2 : 1)),
                  child: Text('${c.avatar} ${c.name}', style: TextStyle(
                    color: isSel ? const Color(0xFF00E676) : Colors.white70,
                    fontSize: isTV ? 18 : 14, fontWeight: isSel ? FontWeight.bold : FontWeight.normal)),
                ),
              );
            }).toList()),
            ...
            const SizedBox(height: 8),
            Center(child: SizedBox(width: isTV ? 140 : 120, child: TvTextField(
              controller: linesCtrl, keyboardType: TextInputType.number, textAlign: TextAlign.center,
              style: TextStyle(color: const Color(0xFF00E676), fontSize: isTV ? 32 : 28, fontWeight: FontWeight.w900),
              inputFormatters: [FilteringTextInputFormatter.digitsOnly],
              onChanged: (_) => setDialogState(() {}),
              decoration: InputDecoration(filled: true, fillColor: Colors.white.withOpacity(0.1),
                border: OutlineInputBorder(borderRadius: BorderRadius.circular(12), borderSide: BorderSide.none)),
            ))),
            Center(child: Text('max: $maxLines', style: TextStyle(color: Colors.white.withOpacity(0.3), fontSize: isTV ? 14 : 11))),
            const SizedBox(height: 8),
            Center(child: Wrap(spacing: 8, children: [1, 3, 5, 10, 20].where((n) => n <= maxLines).map((n) =>
              TvFocusWrapper(
            ...
            SizedBox(height: isTV ? 20 : 16),
            Text('Service demand\u00E9 en \u00E9change :', style: TextStyle(color: Colors.white70, fontSize: isTV ? 18 : 13)),
            const SizedBox(height: 8),
            TvTextField(
              controller: serviceCtrl, onChanged: (_) => setDialogState(() {}),
              style: TextStyle(color: Colors.white, fontSize: isTV ? 18 : 14), maxLines: 2,
          ...
          actions: [
            ...
            TvFocusWrapper(
              ...
              child: ElevatedButton(
                ...
                child: Text('Proposer la vente', style: TextStyle(fontWeight: FontWeight.bold, fontSize: isTV ? 18 : 14)),
              ),
            ),
          ],
        );
  ...
  void _showParentValidationDialog(BuildContext context, FamilyProvider provider, TradeModel trade) {
    ...
    showDialog(context: context, builder: (ctx) {
      return AlertDialog(
        ...
        title: Row(children: [
          Icon(Icons.gavel_rounded, color: const Color(0xFF7C4DFF), size: isTV ? 26 : 22),
          const SizedBox(width: 8),
          Text('Validation parent', style: TextStyle(color: const Color(0xFF7C4DFF), fontSize: isTV ? 24 : 18)),
        ]),
        content: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.start, children: [
          Container(
            width: double.infinity, padding: EdgeInsets.all(isTV ? 16 : 12),
            ...
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              const SizedBox(height: 6),
              Text('\u{1F4BC} ${trade.serviceDescription}',
                style: TextStyle(color: Colors.white70, fontSize: isTV ? 16 : 13)),
            ]),
          ),
          SizedBox(height: isTV ? 20 : 16),
          Text('Note optionnelle :', style: TextStyle(color: Colors.white70, fontSize: isTV ? 18 : 13)),
          const SizedBox(height: 8),
          TvTextField(controller: noteCtrl, style: TextStyle(color: Colors.white, fontSize: isTV ? 18 : 14), maxLines: 2,
            decoration: InputDecoration(
        ...
        actions: [
          ...
          TvFocusWrapper(
            ...
            child: ElevatedButton.icon(
              ...
              icon: Icon(Icons.check_circle_rounded, size: isTV ? 22 : 18),
              label: Text('Valider la vente', style: TextStyle(fontSize: isTV ? 18 : 14)),
  ...
  Widget _buildSectionTitle(String title, Color color) {
    return Text(title, style: TextStyle(color: color, fontSize: isTV ? 18 : 13, fontWeight: FontWeight.w700, letterSpacing: 0.5));
  }
  ...
  Widget _buildBadgeCount(int count) {
    return Container(
      ...
      child: Text('$count', style: TextStyle(color: Colors.black, fontSize: isTV ? 14 : 11, fontWeight: FontWeight.w900)),
    );
  }
}