...
class _DashboardScreenState extends State<DashboardScreen>
  ...
  List<Map<String, dynamic>> _getWeekNotesForChild(
    ...
    for (int i = 0; i < 5; i++) {
      ...
      final dayHistory = provider.history
          .where((h) =>
              h.childId == childId &&
                        ...
                        children: [
                          _buildQuickAction(
                            ...
                            PinGuard.guardNavigation(
                                context, const PunishmentLinesScreen());
                          }),
                          const SizedBox(width: 8),
                          _buildQuickAction(
                          ...
                          const SizedBox(width: 8),
                          _buildQuickAction(Icons.tv_rounded, 'Ecran',
                        ...
                        child: Column(
                          ...
                          children: [
                            ...
                            ...activeTrades.map((trade) =>
                                _buildInteractiveTradeCard(
                                    trade, provider)),
                          ],
                        ),
                      ),

                    // ─── Children cards with week notes & screen time ───
                    if (provider.children.isNotEmpty)
                              ...
                              children: [
                                ...
                                TvFocusWrapper(
                                  ...
                                  borderRadius: const BorderRadius.all(
                                      Radius.circular(12)),
                                  child: _buildWeekNotesBar(
                                      weekNotes, child.name),
                                ),
  ...
  Widget _buildInteractiveTradeCard(
      TradeModel trade, FamilyProvider provider) {
                ...
                Expanded(
                    child: ElevatedButton.icon(
                        ...
                        icon: Icon(
                            ...
                            size: 16),
                        label: Text(trade.isServiceDone
                            ? 'Valider'
                            : statusLabel),
                        style: ElevatedButton.styleFrom(
                            ...
                            foregroundColor: trade.isServiceDone
                                ? Colors.white
                                : Colors.orange,
                            shape: RoundedRectangleBorder(
                                borderRadius:
  ...
  void _showHistoryDetail(BuildContext context, HistoryEntry entry,
    ...
    final color =
        isPositive ? const Color(0xFF00E676) : const Color(0xFFFF1744);
    final dateStr =
        '${entry.date.day.toString().padLeft(2, '0')}/${entry.date.month.toString().padLeft(2, '0')}/${entry.date.year} a ${entry.date.hour.toString().padLeft(2, '0')}:${entry.date.minute.toString().padLeft(2, '0')}';
      ...
      builder: (ctx) => Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            Row(children: [
              Expanded(
                  child: _detailChip(Icons.category_rounded,
                      'Categorie', entry.category, const Color(0xFF448AFF))),
              const SizedBox(width: 10),
              Expanded(
                  child: _detailChip(Icons.access_time_rounded,
                      'Date', dateStr, Colors.amber)),
            ]),
  ...
  Widget _buildScreenTimeMini(
      String childId, int totalMinutes, FamilyProvider provider) {
    final satMin = provider.getSaturdayMinutes(childId);
    final sunMin = provider.getSundayMinutes(childId);
    final progress =
        totalMinutes > 0 ? (totalMinutes / 720).clamp(0.0, 1.0) : 0.0;
    Color barColor;
    if (progress >= 0.5) {
                ...
                children: [
                  Container(
                      ...
                      child: Column(
                          ...
                          children: [
                            Text(
                                '${seller?.name ?? "?"} vend ${trade.immunityLines} ligne${trade.immunityLines > 1 ? 's' : ''} a ${buyer?.name ?? "?"}',
                                style: const TextStyle(
                                    color: Colors.white,
                                    fontWeight: FontWeight.w600)),
                            const SizedBox(height: 6),
                            Text(
                                'Service : ${trade.serviceDescription}',
                                style: TextStyle(
                                    color: Colors.white
                    ...
                    ScaffoldMessenger.of(context).showSnackBar(
                        SnackBar(
                            content: const Row(children: [
                              ...
                              SizedBox(width: 8),
                              Text(
                                  'Vente validee ! Immunites transferees.')
                            ]),
                            backgroundColor:
                                const Color(0xFF7C4DFF),
                            behavior: SnackBarBehavior.floating,
      ...
      child: Row(children: [
        ...
        ...notes.map((n) {
          ...
          if (hasNote && grade != null) {
            if (grade >= 16) {
              noteColor = const Color(0xFF00E676);
            } else if (grade >= 12) {
              noteColor = const Color(0xFF448AFF);
            } else if (grade >= 10) {
              noteColor = Colors.orange;
            } else {
              noteColor = const Color(0xFFFF1744);
            }
          }
          return Expanded(
              child: Container(
  ...
  // ──────────────────────────────────────────────
  // CORRIGÉ : Column fixe → DraggableScrollableSheet + TvFocusWrapper
  // ──────────────────────────────────────────────
  ...
  // ──────────────────────────────────────────────
  // CORRIGÉ : Historique complet avec TvFocusWrapper
  // ──────────────────────────────────────────────
                    ...
                    itemBuilder: (_, i) {
                      ...
                      return TvFocusWrapper(
                        onTap: () {
                          Navigator.pop(ctx);
                          _showHistoryDetail(
                              context, entry, child, provider);
                        },
                        focusBorderColor: entry.isBonus
                            ? const Color(0xFF00E676)
                            : const Color(0xFFFF1744),
                        borderRadius: const BorderRadius.all(
                            Radius.circular(12)),
