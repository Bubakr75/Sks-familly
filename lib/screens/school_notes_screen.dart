...
class _SchoolNotesScreenState extends State<SchoolNotesScreen> with SingleTickerProviderStateMixin {
  ...
  bool _hasNoteForChildOnDate(List<dynamic> history, String childId, DateTime date) {
    return history.any((h) => h.childId == childId && h.category == 'school_note' && _isSameDay(h.date, date));
  }
        ...
        children: [
          ...
          Row(
            ...
            children: [
              IconButton(
                ...
                onPressed: () => setState(() => _selectedMonth = DateTime(year, month - 1)),
              ),
          ...
          Row(
            children: _dayNames.map((d) => Expanded(
              child: Center(child: Text(d, style: TextStyle(color: Colors.grey[600], fontSize: 10, fontWeight: FontWeight.w600))),
            )).toList(),
          ),
          const SizedBox(height: 8),
          // Grille des jours
                  ...
                  child: GestureDetector(
                    ...
                    child: Container(
                      ...
                      decoration: BoxDecoration(
                        ...
                        color: bgColor ?? (isToday ? _blueColor.withValues(alpha: 0.1) : null),
                        border: isToday ? Border.all(color: _blueColor.withValues(alpha: 0.5), width: 1.5) : null,
                      ),
                      child: Column(
                        mainAxisAlignment: MainAxisAlignment.center,
                        children: [
                          ...
                          if (dayGrade != null)
                            Text('${dayGrade.toStringAsFixed(0)}', style: TextStyle(color: textColor, fontSize: 9, fontWeight: FontWeight.w700)),
                        ],
                      ),
                    ),
                  ),
  ...
  Widget _buildMonthAverage(List<dynamic> history) {
    ...
    final monthNotes = history.where((h) =>
        h.childId == _selectedChildId &&
        h.date.year == year &&
        h.date.month == month).toList();

    if (monthNotes.isEmpty) {
      return GlassCard(
        child: Center(child: Text('Aucune note ce mois-ci', style: TextStyle(color: Colors.grey[500], fontSize: 13))),
      );
    }

    double total = 0;
    int count = 0;
    for (final h in monthNotes) {
      final grade = _extractGrade(h.reason);
      if (grade != null) { total += grade; count++; }
    }
          ...
          Expanded(
            child: Column(
              ...
              children: [
                ...
                Row(children: [
                  Text(scale['emoji'] as String, style: const TextStyle(fontSize: 16)),
                  const SizedBox(width: 6),
                  Text(scale['label'] as String, style: TextStyle(color: scale['color'] as Color, fontWeight: FontWeight.w700, fontSize: 14)),
                ]),
                const SizedBox(height: 2),
    ...
    return Column(
      ...
      children: [
        Padding(
          ...
          child: NeonText(text: 'Historique du mois', fontSize: 14, color: Colors.white, glowIntensity: 0.15),
        ),
        ...monthNotes.map((h) {
          final grade = _extractGrade(h.reason) ?? 0;
          final scale = getScaleForGrade(grade);
          ...
          return Container(
            ...
            decoration: BoxDecoration(
              borderRadius: BorderRadius.circular(14),
              color: (scale['color'] as Color).withValues(alpha: 0.06),
              border: Border.all(color: (scale['color'] as Color).withValues(alpha: 0.15)),
            ),
            child: Row(
              children: [
                Container(
                  ...
                  decoration: BoxDecoration(
                    ...
                    border: Border.all(color: (scale['color'] as Color).withValues(alpha: 0.3)),
                  ),
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      if (comment.isNotEmpty)
                        Text(comment, style: TextStyle(color: Colors.grey[500], fontSize: 11, fontStyle: FontStyle.italic), maxLines: 1, overflow: TextOverflow.ellipsis),
  ...
  Widget _buildGradeScaleCard() {
    return GlassCard(
      child: Column(
        ...
        children: [
          Row(children: [
            const Icon(Icons.info_outline_rounded, color: _cyanColor, size: 18),
            const SizedBox(width: 8),
  ...
  void _showDayDetail(BuildContext context, DateTime date, List<dynamic> history) {
    ...
    showDialog(
      ...
      builder: (ctx) => AlertDialog(
        ...
        title: Row(children: [
          Text(scale['emoji'] as String, style: const TextStyle(fontSize: 24)),
          const SizedBox(width: 10),
          Text('${date.day}/${date.month}/${date.year}', style: const TextStyle(color: Colors.white, fontSize: 16)),
        ]),
        content: Column(
          mainAxisSize: MainAxisSize.min,
          children: [
  ...
  void _showAddNote(BuildContext context) {
    ...
    // Verifier si une note existe deja aujourd'hui
    final today = DateTime.now();
    bool alreadyHasNote = false;
    if (selectedChildId != null) {
      alreadyHasNote = provider.history.any((h) => h.childId == selectedChildId && h.category == 'school_note' && _isSameDay(h.date, today));
    }
    ...
    showModalBottomSheet(
      ...
      builder: (ctx) => StatefulBuilder(
        builder: (ctx, setState) {
          // Mettre a jour le check quand on change d'enfant
          if (selectedChildId != null) {
            alreadyHasNote = provider.history.any((h) => h.childId == selectedChildId && h.category == 'school_note' && _isSameDay(h.date, today));
          }
              ...
              child: Column(
                ...
                children: [
                  ...
                  Row(children: [
                    Container(
                      width: 40, height: 40,
                      decoration: BoxDecoration(color: _blueColor.withValues(alpha: 0.12), shape: BoxShape.circle, border: Border.all(color: _blueColor.withValues(alpha: 0.3))),
                  ...
                  TextField(
                    ...
                    decoration: InputDecoration(
                      ...
                      hintText: 'Ex: Sage, a bien ecoute...',
                  ...
                  if (previewGrade != null)
                    AnimatedContainer(
                      ...
                      decoration: BoxDecoration(
                        ...
                        gradient: LinearGradient(colors: [(previewScale['color'] as Color).withValues(alpha: 0.12), (previewScale['color'] as Color).withValues(alpha: 0.04)]),
                    ...
                    child: FilledButton.icon(
                      ...
                      onPressed: () {
                        if (previewGrade != null && selectedChildId != null) {
                          ...
                          provider.addPoints(selectedChildId!, previewPoints, reason, 'school_note', isBonus: previewPoints >= 0);
                          Navigator.pop(ctx);
