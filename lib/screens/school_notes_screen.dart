...
// ═══════════════════════════════════════════════════════════
//  CAHIER QUI S'OUVRE (version scolaire)
// ═══════════════════════════════════════════════════════════
...
class _SchoolNotebookOpenState extends State<_SchoolNotebookOpen> with SingleTickerProviderStateMixin {
  ...
  Widget build(BuildContext context) {
    return AnimatedBuilder(
      animation: _ctrl,
      builder: (context, _) => Center(child: SizedBox(width: 260, height: 300, child: Stack(alignment: Alignment.center, children: [
        // Page blanche
        Opacity(opacity: _pagesFade.value, child: Container(width: 220, height: 270,
...
class _StarsAnimationState extends State<_StarsAnimation> with SingleTickerProviderStateMixin {
  late AnimationController _ctrl;
    ...
    return AnimatedBuilder(
      ...
      builder: (context, _) {
        ...
        return Stack(alignment: Alignment.center, children: [
          ...
          Row(mainAxisSize: MainAxisSize.min, children: List.generate(widget.starCount, (i) {
          ...
          // Texte
          if (t > 0.5)
            Positioned(bottom: MediaQuery.of(context).size.height * 0.32, child: Opacity(
...
class _SchoolNotesScreenState extends State<SchoolNotesScreen> {
              ...
              return Container(
                ...
                child: ListView(controller: scrollController, padding: const EdgeInsets.all(20), children: [
                  ...
                  Row(mainAxisAlignment: MainAxisAlignment.center, children: [
                    ...
                    const Text('Nouvelle note', style: TextStyle(color: Colors.white, fontSize: 20, fontWeight: FontWeight.bold)),
                  ]),
                  ...
                  Row(mainAxisAlignment: MainAxisAlignment.center, children: [
                    TvFocusWrapper(onTap: () { if (value > 0) setModalState(() => value--); },
                      child: GestureDetector(onTap: () { if (value > 0) setModalState(() => value--); },
                    ...
                    // Note animée avec couleur dynamique
                    TweenAnimationBuilder<double>(
                      tween: Tween<double>(begin: value.toDouble(), end: value.toDouble()),
                      duration: const Duration(milliseconds: 200),
                      builder: (context, val, _) {
                        final percent = maxValue > 0 ? val / maxValue * 100 : 0.0;
                        return Text('${val.round()} / $maxValue', style: TextStyle(
                          color: percent >= 50 ? Colors.greenAccent : Colors.redAccent,
                          fontSize: 32, fontWeight: FontWeight.bold));
                      },
                    ),
                    const SizedBox(width: 16),
                    TvFocusWrapper(onTap: () { if (value < maxValue) setModalState(() => value++); },
                      child: GestureDetector(onTap: () { if (value < maxValue) setModalState(() => value++); },
                  ...
                  Row(mainAxisAlignment: MainAxisAlignment.center, children: [10, 20, 40, 100].map((val) {
                    return Padding(padding: const EdgeInsets.symmetric(horizontal: 4), child: TvFocusWrapper(
                      onTap: () => setModalState(() { maxValue = val; if (value > maxValue) value = maxValue; }),
  ...
  void _submitNote(BuildContext ctx, FamilyProvider provider, String subject, String customSubject, int value, int maxValue) async {
    ...
    // Animation étoiles selon la note
    await showStarsAnimation(this.context, percent);
  ...
  Widget build(BuildContext context) {
    return Consumer<FamilyProvider>(
      builder: (context, provider, _) {
        ...
        // Moyenne animée
        final avgPercent = notes.isNotEmpty ? notes.fold<double>(0, (sum, n) => sum + (n.maxValue > 0 ? n.value / n.maxValue * 100 : 0)) / notes.length : 0.0;
            ...
            body: Column(children: [
              ...
              if (notes.isNotEmpty)
                Container(
                  ...
                  child: Row(children: [
                    // Étoiles miniatures
                    Row(children: List.generate(_percentToStars(avgPercent), (i) => const Padding(padding: EdgeInsets.only(right: 2), child: Text('⭐', style: TextStyle(fontSize: 14))))),
                    const SizedBox(width: 10),
                    Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                      const Text('Moyenne générale', style: TextStyle(color: Colors.white70, fontSize: 12)),
                      TweenAnimationBuilder<double>(
                        tween: Tween<double>(begin: 0, end: avgPercent),
                        duration: const Duration(milliseconds: 800),
                        builder: (context, val, _) => Text('${val.round()}%', style: TextStyle(color: val >= 50 ? Colors.greenAccent : Colors.redAccent, fontSize: 22, fontWeight: FontWeight.bold)),
                      ),
                    ])),
                    Text('${notes.length} note${notes.length > 1 ? 's' : ''}', style: TextStyle(color: Colors.white.withOpacity(0.4), fontSize: 12)),
                  ]),
                ),
              Expanded(
                child: notes.isEmpty
                            ...
                            child: GestureDetector(
                              ...
                              child: GlassCard(
                                child: Padding(
                                  ...
                                  child: Row(children: [
                                    // Cercle pourcentage animé
                                    TweenAnimationBuilder<double>(
                                      tween: Tween<double>(begin: 0, end: percentage),
                                      duration: Duration(milliseconds: 600 + index * 100),
                                      curve: Curves.easeOutCubic,
                                      builder: (context, val, _) => Container(width: 48, height: 48,
                                        decoration: BoxDecoration(shape: BoxShape.circle, color: (isGood ? Colors.greenAccent : Colors.redAccent).withOpacity(0.15)),
                                        child: Center(child: Text('${val.round()}%', style: TextStyle(color: isGood ? Colors.greenAccent : Colors.redAccent, fontWeight: FontWeight.bold, fontSize: 13)))),
                                    ),
    ...
    showModalBottomSheet(context: context, backgroundColor: Colors.transparent,
      builder: (ctx) => Container(
        ...
        child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          const SizedBox(height: 16),
          // Étoiles animées dans le détail
          Center(child: Row(mainAxisSize: MainAxisSize.min, children: List.generate(stars, (i) {
            return TweenAnimationBuilder<double>(
              ...
              builder: (context, val, child) => Transform.scale(scale: val, child: child),
              child: const Padding(padding: EdgeInsets.symmetric(horizontal: 4), child: Text('⭐', style: TextStyle(fontSize: 28))),
            );
          }))),
          const SizedBox(height: 16),
