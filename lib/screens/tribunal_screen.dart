...
class _TribunalScreenState extends State<TribunalScreen> with SingleTickerProviderStateMixin {
  ...
  Widget _avatarText(ChildModel? child) {
    return Container(
      ...
      child: Center(
        child: Text(
          child?.avatar.isNotEmpty == true ? child!.avatar : '\u{1F464}',
      ...
      builder: (ctx) => DraggableScrollableSheet(
        ...
        builder: (_, sc) => ListView(
          ...
          children: [
            ...
            const SizedBox(height: 8),
            Center(
              child: Text(tc.title, style: const TextStyle(color: Colors.white, fontSize: 22, fontWeight: FontWeight.w800), textAlign: TextAlign.center),
            ),
            const SizedBox(height: 8),
            Center(
              child: Container(
            ...
            Row(
              children: [
                ...
                const Padding(
                  ...
                  child: Text('VS', style: TextStyle(color: Colors.white54, fontWeight: FontWeight.w900, fontSize: 20)),
                ),
            ...
            if (tc.prosecutionLawyer != null || tc.defenseLawyer != null) ...[
              ...
              if (tc.prosecutionLawyer != null)
                _buildParticipantTile(tc.prosecutionLawyer!, provider, "Avocat de l'accusation", _courtBlue),
              if (tc.defenseLawyer != null)
                _buildParticipantTile(tc.defenseLawyer!, provider, 'Avocat de la defense', _courtPurple),
              const SizedBox(height: 16),
            ],
            // Temoins
            ...
            if (tc.verdict != null) ...[
              ...
              Container(
                ...
                decoration: BoxDecoration(
                  ...
                  border: Border.all(
                    color: tc.verdict == TribunalVerdict.guilty ? _courtRed.withValues(alpha: 0.3) : tc.verdict == TribunalVerdict.innocent ? _courtGreen.withValues(alpha: 0.3) : Colors.grey.withValues(alpha: 0.3),
                  ),
                ),
                child: Column(
                  children: [
  ...
  Widget _buildDetailParty(ChildModel? child, String role, Color color, int points) {
    return Container(
      ...
      child: Column(
        children: [
          if (child != null && child.hasPhoto)
            ClipOval(child: Image.memory(base64Decode(child.photoBase64), width: 50, height: 50, fit: BoxFit.cover,
                errorBuilder: (_, __, ___) => _bigAvatar(child)))
          else
            _bigAvatar(child),
          const SizedBox(height: 8),
          ...
          if (points != 0) ...[
            ...
            Container(
              ...
              decoration: BoxDecoration(
                color: (points > 0 ? _courtGreen : _courtRed).withValues(alpha: 0.15),
                borderRadius: BorderRadius.circular(6),
              ),
              child: Text(
                '${points > 0 ? '+' : ''}$points pts',
                style: TextStyle(color: points > 0 ? _courtGreen : _courtRed, fontWeight: FontWeight.w800, fontSize: 12),
              ),
            ),
          ],
        ],
      ...
      child: Row(
        children: [
          ...
          if (participant.testimonyVerified != null)
            Icon(
              participant.testimonyVerified! ? Icons.check_circle : Icons.cancel,
              color: participant.testimonyVerified! ? _courtGreen : _courtRed,
              size: 20,
            ),
          if (participant.pointsAwarded != 0) ...[
            const SizedBox(width: 8),
            Text(
              '${participant.pointsAwarded > 0 ? '+' : ''}${participant.pointsAwarded}',
              style: TextStyle(
                color: participant.pointsAwarded > 0 ? _courtGreen : _courtRed,
  ...
  void _showFileComplaint(BuildContext context) {
    ...
    if (provider.children.length < 2) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: const Text('Il faut au moins 2 enfants pour deposer une plainte'), backgroundColor: _courtRed, behavior: SnackBarBehavior.floating, shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12))),
      );
      return;
    }
                ...
                children: [
                  ...
                  const Row(
                    children: [
                      Text('\u{2696}', style: TextStyle(fontSize: 24)),
                      SizedBox(width: 10),
                      Text('Deposer une plainte', style: TextStyle(color: Colors.white, fontSize: 18, fontWeight: FontWeight.w800)),
                    ],
                  ),
                  const SizedBox(height: 20),
                  // Titre
                  ...
                  TextField(
                    ...
                    decoration: InputDecoration(
                      ...
                      hintText: 'Racontez ce qui s\'est passe...',
                      hintStyle: TextStyle(color: Colors.grey[700]),
                      border: OutlineInputBorder(borderRadius: BorderRadius.circular(12)),
                    ),
                  ),
                  ...
                  _childDropdown('Plaignant (qui porte plainte)', plaintiffId, provider.children, _courtBlue, (v) {
                    setState(() {
                      plaintiffId = v;
                      if (prosecutionLawyerId == v) prosecutionLawyerId = null;
                      if (defenseLawyerId == v) defenseLawyerId = null;
                      witnessIds.remove(v);
                    });
                  }),
                  const SizedBox(height: 14),
                  ...
                  if (availableForLawyers.isNotEmpty) ...[
                    const SizedBox(height: 14),
                    _childDropdown("Avocat de l'accusation (optionnel)", prosecutionLawyerId, availableForLawyers, _courtBlue, (v) {
                      setState(() { prosecutionLawyerId = v; witnessIds.remove(v); });
                    }, optional: true),
                    const SizedBox(height: 14),
                    _childDropdown('Avocat de la defense (optionnel)', defenseLawyerId, availableForLawyers.where((c) => c.id != prosecutionLawyerId).toList(), _courtPurple, (v) {
                      setState(() { defenseLawyerId = v; witnessIds.remove(v); });
                    }, optional: true),
                  ],
                  if (availableForWitnesses.isNotEmpty) ...[
                    ...
                    Wrap(
                      ...
                      children: availableForWitnesses.map((c) {
                        ...
                        return FilterChip(
                          selected: isSelected,
                          label: Text('${c.avatar.isEmpty ? "\u{1F466}" : c.avatar} ${c.name}'),
                          selectedColor: Colors.amber.withValues(alpha: 0.2),
                          checkmarkColor: Colors.amber,
                          onSelected: (v) {
                            setState(() {
                              if (v) { witnessIds.add(c.id); } else { witnessIds.remove(c.id); }
                            });
                          },
                        );
                      }).toList(),
                  ...
                  SizedBox(
                    ...
                    child: FilledButton.icon(
                      ...
                      onPressed: (titleCtrl.text.trim().isNotEmpty && descCtrl.text.trim().isNotEmpty && plaintiffId != null && accusedId != null)
                          ? () async {
                              await provider.fileTribunalCase(
                                title: titleCtrl.text.trim(),
                                description: descCtrl.text.trim(),
                                plaintiffId: plaintiffId!,
                                accusedId: accusedId!,
                                prosecutionLawyerId: prosecutionLawyerId,
                                defenseLawyerId: defenseLawyerId,
                                witnessIds: witnessIds.isNotEmpty ? witnessIds : null,
                              );
                              if (ctx.mounted) Navigator.pop(ctx);
                              if (mounted) {
                                ScaffoldMessenger.of(context).showSnackBar(
                                  SnackBar(
                                    content: const Row(children: [Text('\u{2696}', style: TextStyle(fontSize: 18)), SizedBox(width: 8), Text('Plainte deposee !')]),
                                    backgroundColor: _courtRed, behavior: SnackBarBehavior.floating, shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12)),
                                  ),
                                );
                              }
                            }
  ...
  void _showScheduleHearing(BuildContext context, TribunalCase tc, FamilyProvider provider) async {
    ...
    if (mounted) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Row(children: [const Text('\u{1F4C5}', style: TextStyle(fontSize: 18)), const SizedBox(width: 8), Text('Audience prevue le ${_formatDateTime(scheduledDate)}')]),
          backgroundColor: _courtBlue, behavior: SnackBarBehavior.floating, shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12)),
        ),
      );
    }
  }
  ...
  void _showRenderVerdict(BuildContext context, TribunalCase tc, FamilyProvider provider) {
    ...
    for (final w in tc.witnesses) {
      witnessVerified[w.childId] = true;
      witnessPts[w.childId] = 2;
    }
              ...
              child: Column(
                ...
                children: [
                  ...
                  TextField(
                    ...
                    maxLines: 2,
                    decoration: InputDecoration(
                      labelText: 'Explication du verdict',
                      hintText: 'Pourquoi cette decision...',
                      hintStyle: TextStyle(color: Colors.grey[700]),
                      border: OutlineInputBorder(borderRadius: BorderRadius.circular(12)),
                    ),
                  ),
                  ...
                  if (tc.witnesses.isNotEmpty) ...[
                    ...
                    ...tc.witnesses.map((w) {
                      ...
                      return Container(
                        ...
                        padding: const EdgeInsets.all(10),
                        decoration: BoxDecoration(
                          color: (verified ? _courtGreen : _courtRed).withValues(alpha: 0.08),
                          borderRadius: BorderRadius.circular(10),
                          border: Border.all(color: (verified ? _courtGreen : _courtRed).withValues(alpha: 0.2)),
                        ),
                        child: Row(
                          children: [
                            ...
                            const Spacer(),
                            ChoiceChip(
                              label: const Text('Veridique'),
                              selected: verified,
                              selectedColor: _courtGreen.withValues(alpha: 0.2),
                              onSelected: (_) => setState(() => witnessVerified[w.childId] = true),
                            ),
                            const SizedBox(width: 6),
                            ChoiceChip(
                              label: const Text('Faux'),
                              selected: !verified,
                              selectedColor: _courtRed.withValues(alpha: 0.2),
                              onSelected: (_) => setState(() => witnessVerified[w.childId] = false),
                            ),
                          ],
                        ),
                      );
                    ...
                    child: FilledButton.icon(
                      ...
                      onPressed: () async {
                        ...
                        if (mounted) {
                          ScaffoldMessenger.of(context).showSnackBar(
                            SnackBar(
                              content: Row(children: [const Text('\u{1F528}', style: TextStyle(fontSize: 18)), const SizedBox(width: 8), Text('Verdict rendu: ${selectedVerdict == TribunalVerdict.guilty ? "Coupable" : selectedVerdict == TribunalVerdict.innocent ? "Innocent" : "Classe"}')]),
                              backgroundColor: _courtGold, behavior: SnackBarBehavior.floating, shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12)),
                            ),
                          );
                        }
                      },
                      icon: const Icon(Icons.gavel_rounded),
                      label: const Text('Confirmer le verdict', style: TextStyle(fontWeight: FontWeight.w700, color: Colors.black)),
                    ),
                  ),
                ],
              ),
      ...
      child: GestureDetector(
        ...
        child: AnimatedContainer(
          ...
          decoration: BoxDecoration(
            ...
            borderRadius: BorderRadius.circular(12),
            border: Border.all(color: isSelected ? color.withValues(alpha: 0.5) : Colors.white.withValues(alpha: 0.1), width: isSelected ? 2 : 1),
          ),
          child: Column(
            children: [
              Text(emoji, style: const TextStyle(fontSize: 22)),
              const SizedBox(height: 4),
              Text(label, style: TextStyle(color: isSelected ? color : Colors.white54, fontSize: 11, fontWeight: FontWeight.w700)),
            ],
          ),
        ),
      ),
  ...
  Widget _pointsPreview(String name, int points) {
    return Padding(
      ...
      child: Row(
        children: [
          Expanded(child: Text(name, style: TextStyle(color: Colors.white.withValues(alpha: 0.7), fontSize: 13))),
          Text(
            points == 0 ? '0 pts' : '${points > 0 ? '+' : ''}$points pts',
            style: TextStyle(
              color: points > 0 ? _courtGreen : points < 0 ? _courtRed : Colors.grey,
              fontWeight: FontWeight.w800,
              fontSize: 14,
            ),
          ),
  ...
  String _formatDateTime(DateTime date) {
    return '${date.day.toString().padLeft(2, '0')}/${date.month.toString().padLeft(2, '0')} a ${date.hour.toString().padLeft(2, '0')}h${date.minute.toString().padLeft(2, '0')}';
  }
}
