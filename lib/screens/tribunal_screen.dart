...
class _GavelAnimationState extends State<GavelAnimation> with SingleTickerProviderStateMixin {
  ...
  void initState() {
    ...
    _rotation = TweenSequence<double>([
      TweenSequenceItem(tween: Tween(begin: 0, end: -0.5).chain(CurveTween(curve: Curves.easeOut)), weight: 40),
      TweenSequenceItem(tween: Tween(begin: -0.5, end: 0.05).chain(CurveTween(curve: Curves.bounceOut)), weight: 60),
    ]).animate(_ctrl);
    _scale = TweenSequence<double>([
      TweenSequenceItem(tween: Tween(begin: 1.0, end: 0.9), weight: 40),
      TweenSequenceItem(tween: Tween(begin: 0.9, end: 1.15).chain(CurveTween(curve: Curves.elasticOut)), weight: 60),
    ]).animate(_ctrl);
...
class _GuiltyFlashAnimationState extends State<GuiltyFlashAnimation> with SingleTickerProviderStateMixin {
  ...
  void initState() {
    ...
    _opacity = TweenSequence<double>([
      TweenSequenceItem(tween: Tween(begin: 0, end: 0.6), weight: 10),
      TweenSequenceItem(tween: Tween(begin: 0.6, end: 0), weight: 15),
      TweenSequenceItem(tween: Tween(begin: 0, end: 0.4), weight: 10),
      TweenSequenceItem(tween: Tween(begin: 0.4, end: 0), weight: 15),
      TweenSequenceItem(tween: Tween(begin: 0, end: 0.2), weight: 10),
...
class _VerdictAnimationDialogState extends State<_VerdictAnimationDialog> with TickerProviderStateMixin {
  ...
  void initState() {
    ...
    _textOpacity = Tween<double>(begin: 0, end: 1).animate(CurvedAnimation(parent: _textCtrl, curve: Curves.easeIn));
            ...
            child: SingleChildScrollView(
              child: Column(
                ...
                children: [
                  ...
                  TextField(controller: descCtrl, style: const TextStyle(color: Colors.white), maxLines: 3, decoration: InputDecoration(labelText: 'Description des faits', hintText: 'Racontez ce qui s\'est passé...', hintStyle: TextStyle(color: Colors.grey[700]), border: OutlineInputBorder(borderRadius: BorderRadius.circular(12)))),
                  const SizedBox(height: 14),
                  ...
                  if (availableForWitnesses.isNotEmpty) ...[
                    ...
                    Wrap(
                      ...
                      children: availableForWitnesses.map((c) {
                        ...
                        return FilterChip(selected: isSelected, label: Text('${c.avatar.isEmpty ? "\u{1F466}" : c.avatar} ${c.name}'), selectedColor: Colors.amber.withOpacity(0.2), checkmarkColor: Colors.amber, onSelected: (v) { setState(() { if (v) { witnessIds.add(c.id); } else { witnessIds.remove(c.id); } }); });
                      }).toList(),
                    ),
                  ],
                  const SizedBox(height: 20),
