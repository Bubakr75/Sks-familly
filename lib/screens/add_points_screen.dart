...
class _AddPointsScreenState extends State<AddPointsScreen> {
  ...
  final List<String> _bonusReasons = ['Devoirs faits', 'Chambre rangee', 'Bon comportement', 'Aide menage', 'Lecture', 'Sport', 'Politesse', 'Initiative'];
  final List<String> _penaltyReasons = ['Desobeissance', 'Bagarre', 'Mensonge', 'Ecran abusif', 'Desordre', 'Retard', 'Insolence', 'Devoirs non faits'];

  @override
  void dispose() { _customReasonController.dispose(); super.dispose(); }

  List<String> get _currentReasons => _isBonus ? _bonusReasons : _penaltyReasons;

  Future<void> _submitPoints() async {
    if (_selectedChildId == null) { ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Selectionnez un enfant'))); return; }
    final finalReason = _reason.isNotEmpty ? _reason : _customReason.isNotEmpty ? _customReason : null;
    if (finalReason == null || finalReason.isEmpty) { ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Indiquez une raison'))); return; }
    final provider = context.read<FamilyProvider>();
    String? proofBase64;
    if (_proofPhoto != null) { final bytes = await _proofPhoto!.readAsBytes(); proofBase64 = base64Encode(bytes); }
    provider.addPoints(_selectedChildId!, _points, finalReason, category: _isBonus ? 'bonus' : 'penalty', isBonus: _isBonus, proofPhotoBase64: proofBase64);
    HapticFeedback.mediumImpact();
    if (mounted) { ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('${_isBonus ? '+' : '-'}$_points points attribues !'), backgroundColor: _isBonus ? Colors.green : Colors.redAccent)); }
    setState(() { _reason = ''; _customReason = ''; _customReasonController.clear(); _points = 1; _proofPhoto = null; });
  }
  ...
  Future<void> _pickPhoto() async {
    showModalBottomSheet(context: context, backgroundColor: Colors.grey[900],
      ...
      builder: (ctx) => SafeArea(child: Padding(padding: const EdgeInsets.all(16), child: Column(mainAxisSize: MainAxisSize.min, children: [
        ...
        TvFocusWrapper(onTap: () async { Navigator.pop(ctx); final picked = await ImagePicker().pickImage(source: ImageSource.camera); if (picked != null) setState(() => _proofPhoto = File(picked.path)); },
        ...
        TvFocusWrapper(onTap: () async { Navigator.pop(ctx); final picked = await ImagePicker().pickImage(source: ImageSource.gallery); if (picked != null) setState(() => _proofPhoto = File(picked.path)); },
        ...
        if (_proofPhoto != null)
          TvFocusWrapper(onTap: () { setState(() => _proofPhoto = null); Navigator.pop(ctx); },
        ...
        body: SingleChildScrollView(
          ...
          child: Column(
            ...
            children: [
              ...
              // Wrap au lieu de ListView horizontal pour le focus TV
              Wrap(
                    ...
                    child: AnimatedContainer(
                      ...
                      child: Column(mainAxisSize: MainAxisSize.min, children: [
                        CircleAvatar(radius: 18, backgroundColor: Colors.cyanAccent.withOpacity(0.3),
                          ...
                          child: !child.hasPhoto ? Text(child.name.isNotEmpty ? child.name[0].toUpperCase() : '?', style: const TextStyle(color: Colors.white, fontWeight: FontWeight.bold)) : null),
              ...
              Row(children: [
                Expanded(child: TvFocusWrapper(onTap: () => setState(() => _isBonus = true),
                  child: AnimatedContainer(duration: const Duration(milliseconds: 200), padding: const EdgeInsets.symmetric(vertical: 14),
                    ...
                    child: Center(child: Row(mainAxisSize: MainAxisSize.min, children: [
                      Icon(Icons.thumb_up_rounded, color: _isBonus ? Colors.greenAccent : Colors.white38, size: 20), const SizedBox(width: 8),
                      Text('Bonus', style: TextStyle(color: _isBonus ? Colors.greenAccent : Colors.white38, fontWeight: FontWeight.w600)),
                    ]))))),
                const SizedBox(width: 12),
                Expanded(child: TvFocusWrapper(onTap: () => setState(() => _isBonus = false),
                  child: AnimatedContainer(duration: const Duration(milliseconds: 200), padding: const EdgeInsets.symmetric(vertical: 14),
                    ...
                    child: Center(child: Row(mainAxisSize: MainAxisSize.min, children: [
                      Icon(Icons.thumb_down_rounded, color: !_isBonus ? Colors.redAccent : Colors.white38, size: 20), const SizedBox(width: 8),
                      Text('Penalite', style: TextStyle(color: !_isBonus ? Colors.redAccent : Colors.white38, fontWeight: FontWeight.w600)),
                    ]))))),
              ]),
              const SizedBox(height: 24),
              ...
              Wrap(spacing: 8, runSpacing: 8, children: _currentReasons.map((r) {
                ...
                return TvFocusWrapper(
                  onTap: () => setState(() { _reason = isSelected ? '' : r; if (_reason.isNotEmpty) { _customReason = ''; _customReasonController.clear(); } }),
                  child: AnimatedContainer(duration: const Duration(milliseconds: 200), padding: const EdgeInsets.symmetric(horizontal: 14, vertical: 8),
                    decoration: BoxDecoration(
                      color: isSelected ? (_isBonus ? Colors.greenAccent.withOpacity(0.25) : Colors.redAccent.withOpacity(0.25)) : Colors.white.withOpacity(0.06),
                      borderRadius: BorderRadius.circular(20),
                      border: Border.all(color: isSelected ? (_isBonus ? Colors.greenAccent : Colors.redAccent) : Colors.white24)),
                    child: Text(r, style: TextStyle(color: isSelected ? (_isBonus ? Colors.greenAccent : Colors.redAccent) : Colors.white70, fontSize: 13))),
                );
              }).toList()),
              const SizedBox(height: 16),
              TextField(
                controller: _customReasonController, style: const TextStyle(color: Colors.white),
              ...
              Row(mainAxisAlignment: MainAxisAlignment.center, children: [
                TvFocusWrapper(onTap: () { if (_points > 1) setState(() => _points--); },
                  child: Container(width: 48, height: 48, decoration: BoxDecoration(shape: BoxShape.circle, color: Colors.white.withOpacity(0.1), border: Border.all(color: Colors.white24)), child: const Icon(Icons.remove, color: Colors.white70))),
                ...
                const SizedBox(width: 24),
                TvFocusWrapper(onTap: () { if (_points < 50) setState(() => _points++); },
                  child: Container(width: 48, height: 48, decoration: BoxDecoration(shape: BoxShape.circle, color: Colors.white.withOpacity(0.1), border: Border.all(color: Colors.white24)), child: const Icon(Icons.add, color: Colors.white70))),
              ]),
              const SizedBox(height: 12),
              Row(mainAxisAlignment: MainAxisAlignment.center, children: [1, 2, 5, 10].map((val) {
                return Padding(padding: const EdgeInsets.symmetric(horizontal: 4), child: TvFocusWrapper(
                  ...
                  child: Container(padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 8),
                    decoration: BoxDecoration(borderRadius: BorderRadius.circular(20), color: _points == val ? Colors.cyanAccent.withOpacity(0.2) : Colors.white.withOpacity(0.06), border: Border.all(color: _points == val ? Colors.cyanAccent : Colors.white24)),
