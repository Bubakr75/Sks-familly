...
class _ChildDashboardScreenState extends State<ChildDashboardScreen>
  ...
  Widget _buildProfileTab(dynamic child, FamilyProvider provider) {
    ...
    // ✅ CORRIGÉ : utiliser les listes directement avec .where au lieu de méthodes inexistantes
    final activePunishments = provider.punishments.where((p) => p.childId == widget.childId && !p.isCompleted).length;
          ...
          child: Padding(
            ...
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              Wrap(spacing: 8, runSpacing: 8, children: [
                ...
                _quickAction(emoji: '✍️', label: 'Punitions', onTap: () => Navigator.push(context, SlidePageRoute(page: PunishmentLinesScreen(childId: widget.childId)))),
                _quickAction(emoji: '🛡️', label: 'Immunités', onTap: () => Navigator.push(context, SlidePageRoute(page: const ImmunityLinesScreen()))),
  ...
  // ✅ CORRIGÉ : utiliser les méthodes qui existent dans FamilyProvider
  ...
  Widget _buildHistoryTab(FamilyProvider provider) {
    ...
    return ListView.builder(
      ...
      itemBuilder: (_, index) {
        ...
        final displayReason = entry.reason.contains('|') ? entry.reason.split('|').first : entry.reason;
        return Padding(
  ...
  // ── ONGLET 4 — BADGES ──
  // ✅ CORRIGÉ : utiliser BadgeModel.defaultBadges + provider.customBadges
