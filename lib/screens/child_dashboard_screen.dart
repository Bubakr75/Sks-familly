...
class _ChildDashboardScreenState extends State<ChildDashboardScreen>
  ...
  Map<String, dynamic> _calculateScreenTime(FamilyProvider provider) {
    ...
    final maxWeekendTotal = 360; // 3h x 2 jours = 6h = 360 min
              ...
              children: [
                if (child.hasPhoto)
                  Positioned.fill(
                    child: Opacity(
                      opacity: 0.15,
                      child: Image.memory(base64Decode(child.photoBase64), fit: BoxFit.cover, gaplessPlayback: true,
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          Row(
            children: [
              Container(
                ...
                child: const Icon(Icons.tv_rounded, color: Color(0xFF7C4DFF), size: 20),
              ),
              const SizedBox(width: 10),
              const Expanded(
                child: Text('Temps d\'ecran du week-end',
          ...
          Text('${_formatMinutes(total)} / ${_formatMinutes(maxMin)} max',
            style: TextStyle(color: Colors.white.withValues(alpha: 0.5), fontSize: 10)),
          const SizedBox(height: 12),
          Row(
            children: [
              Expanded(
                child: _screenTimeDayBox('\u{1F4C5} Samedi', satMin, const Color(0xFF448AFF)),
              ),
              const SizedBox(width: 8),
              Expanded(
                child: _screenTimeDayBox('\u{1F31E} Dimanche', sunMin, const Color(0xFFFF9100)),
              ),
            ],
          ),
          ...
          Container(
            ...
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Text('Detail par jour :', style: TextStyle(color: Colors.white.withValues(alpha: 0.7), fontSize: 11, fontWeight: FontWeight.w600)),
                ...
                ...breakdown.map((d) {
                  ...
                  return Padding(
                    ...
                    child: Row(
                      children: [
                        SizedBox(width: 65, child: Text(d['day'] as String,
                          style: TextStyle(color: Colors.white.withValues(alpha: 0.6), fontSize: 11))),
                        if (hasNote && grade != null) ...[
                          Text('${grade.toStringAsFixed(grade == grade.roundToDouble() ? 0 : 1)}/20',
                            style: TextStyle(color: _gradeColor(grade), fontSize: 11, fontWeight: FontWeight.w700)),
                          const SizedBox(width: 8),
                          Icon(Icons.arrow_forward, size: 10, color: Colors.white.withValues(alpha: 0.3)),
                          const SizedBox(width: 8),
                          Text('+${min}min', style: TextStyle(color: const Color(0xFF00E676), fontSize: 11, fontWeight: FontWeight.w700)),
                        ] else
                          Text('Pas de note', style: TextStyle(color: Colors.white.withValues(alpha: 0.3), fontSize: 11)),
                      ],
                    ),
                  );
                }),
                if (bonus != 0) ...[
                  ...
                  Row(
                    children: [
                      SizedBox(width: 65, child: Text('Parent',
                        style: TextStyle(color: Colors.white.withValues(alpha: 0.6), fontSize: 11))),
          ...
          Row(
            ...
            children: [
              ...
              Text(bonus == 0 ? 'Ajuster' : '${bonus >= 0 ? '+' : ''}${bonus}min',
    ...
    return Container(
      ...
      child: Column(
        children: [
          ...
          const SizedBox(height: 4),
          Text(_formatMinutes(minutes), style: TextStyle(color: color, fontSize: 20, fontWeight: FontWeight.w900)),
          Text('/ 3h max', style: TextStyle(color: Colors.white.withValues(alpha: 0.3), fontSize: 9)),
        ],
      ),
    );
  }
      ...
      child: Column(
        children: [
          Row(children: [
            ...
            const Spacer(),
            Text('Mode Enfant', style: TextStyle(color: Colors.white.withValues(alpha: 0.7), fontSize: 13)),
            const Spacer(),
          ...
          Row(children: [
            Container(
              ...
              decoration: BoxDecoration(shape: BoxShape.circle,
                border: Border.all(color: Colors.amber.withValues(alpha: 0.5), width: 2.5),
            ...
            Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              Container(
                ...
                child: Text(child.levelTitle, style: TextStyle(color: Colors.amber.shade300, fontSize: 12, fontWeight: FontWeight.w600)),
              ),
              const SizedBox(height: 8),
              ClipRRect(borderRadius: BorderRadius.circular(6),
                child: LinearProgressIndicator(value: child.levelProgress, minHeight: 6,
                  backgroundColor: Colors.white.withValues(alpha: 0.1), valueColor: AlwaysStoppedAnimation<Color>(Colors.amber.shade400))),
              const SizedBox(height: 2),
              Text(child.isMaxLevel ? '\u{1F381} Niveau MAX !' : '${child.points} / ${child.nextLevelPoints} pts',
                style: TextStyle(color: child.isMaxLevel ? Colors.amber.shade300 : Colors.white.withValues(alpha: 0.5), fontSize: 10)),
            ])),
          ]),
          const SizedBox(height: 12),
  ...
  Widget _avatarFallback(ChildModel child, double size) {
    return Container(width: size, height: size, color: Colors.white.withValues(alpha: 0.2),
  ...
  Widget _buildWeekNotes(List<Map<String, dynamic>> notes) {
    ...
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        const Row(children: [
          Icon(Icons.school_rounded, color: Color(0xFF448AFF), size: 16), SizedBox(width: 6),
        ...
        Row(children: notes.map((n) {
          ...
          return Expanded(child: Container(
            ...
            decoration: BoxDecoration(borderRadius: BorderRadius.circular(8),
              color: isToday && hasNote ? color.withValues(alpha: 0.2) : isToday ? const Color(0xFF448AFF).withValues(alpha: 0.1) : Colors.white.withValues(alpha: 0.05),
              border: isToday ? Border.all(color: const Color(0xFF448AFF).withValues(alpha: 0.5), width: 1.5) : null),
            child: Column(children: [
              Text((n['day'] as String).substring(0, 3), style: TextStyle(color: Colors.white.withValues(alpha: 0.6), fontSize: 9, fontWeight: FontWeight.w600)),
              ...
              if (hasNote && grade != null) ...[
                ...
                Padding(padding: const EdgeInsets.symmetric(vertical: 3),
                  child: Container(width: 8, height: 8, decoration: BoxDecoration(shape: BoxShape.circle, color: const Color(0xFF448AFF).withValues(alpha: 0.5))))
              else
                Padding(padding: const EdgeInsets.symmetric(vertical: 5),
  ...
  Widget _buildTabBar() {
    return Container(
      ...
      child: TabBar(controller: _tabController,
        ...
        labelStyle: const TextStyle(fontSize: 12, fontWeight: FontWeight.w600), dividerHeight: 0,
  ...
  Widget _buildBadgesTab(ChildModel child, FamilyProvider provider) {
    ...
    if (allBadges.isEmpty) {
      return const Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
        Icon(Icons.emoji_events_outlined, color: Colors.white24, size: 64), SizedBox(height: 16),
            ...
            Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              ClipRRect(borderRadius: BorderRadius.circular(4),
                child: LinearProgressIndicator(value: progress, minHeight: 5,
                  backgroundColor: Colors.white.withValues(alpha: 0.08), valueColor: AlwaysStoppedAnimation(Colors.amber.withValues(alpha: 0.6)))),
              const SizedBox(height: 3),
  ...
  Widget _buildHistoryTab(List<HistoryEntry> history) {
    if (history.isEmpty) {
      return const Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
        Icon(Icons.history, color: Colors.white24, size: 64), SizedBox(height: 16),
