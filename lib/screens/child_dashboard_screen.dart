...
class _ChildDashboardScreenState extends State<ChildDashboardScreen>
  ...
  Future<void> _pickPhoto(ImageSource source) async {
    ...
    if (mounted) {
      final familyProvider = context.read<FamilyProvider>();
      familyProvider.updateChildPhoto(
  ...
  void _removePhoto() {
    final familyProvider = context.read<FamilyProvider>();
    familyProvider.updateChildPhoto(childId: widget.childId, photoBase64: '');
    setState(() {});
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
  ...
  // ──────────────────────────────────────────────
  //  AVATAR LARGE (120px) AVEC GLOW ANIMÉ
  // ──────────────────────────────────────────────
  Widget _buildLargeAvatar(dynamic child, {double size = 120}) {
    ...
    return GestureDetector(
      onTap: () {
        if (child.hasPhoto) {
          _showFullPhoto(context, child.photoBase64);
        } else {
        ...
        builder: (_, __) {
          ...
          return Container(
            ...
            decoration: BoxDecoration(
              ...
              boxShadow: [
                BoxShadow(
                  color: levelColor.withOpacity(0.3 + glowValue * 0.3),
            ...
            child: Container(
              ...
              decoration: BoxDecoration(
                ...
                gradient: SweepGradient(
                  ...
                  transform: GradientRotation(glowValue * math.pi * 2),
                ),
              ),
  ...
  Widget _buildProfileTab(dynamic child, FamilyProvider provider) {
    ...
    final totalBonus =
        history.where((e) => e.isBonus).fold<int>(0, (s, e) => s + e.points);
    final totalPenalty =
        history.where((e) => !e.isBonus).fold<int>(0, (s, e) => s + e.points);
    final punishments = provider.getPunishments(widget.childId);
    final activePunishments = punishments
        .where(
            (p) => (p['completedLines'] ?? 0) < (p['totalLines'] ?? 0))
        .length;
    final immunities = provider.getImmunities(widget.childId);
    final activeImmunities = immunities
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        children: [
          ...
          FadeTransition(
            ...
            child: Container(
              ...
              decoration: BoxDecoration(
                gradient: LinearGradient(
                  colors: [
                    _getLevelColor(child.points).withOpacity(0.3),
                    _getLevelColor(child.points).withOpacity(0.1),
                  ],
                ),
                borderRadius: BorderRadius.circular(20),
                border: Border.all(
                  color: _getLevelColor(child.points).withOpacity(0.5),
                ),
              ),
              child: Text(
                child.levelTitle,
                style: TextStyle(
                  color: _getLevelColor(child.points),
                  fontWeight: FontWeight.bold,
                ),
              ),
            ),
                  ...
                  Wrap(
                    ...
                    children: [
                      ...
                      _quickAction(
                        ...
                        onTap: () => Navigator.push(
                          context,
                          SlidePageRoute(page: const BadgesScreen()),
                        ),
                      ),
                      _quickAction(
                        emoji: '📸',
          ...
          Text(
            child.hasPhoto
                ? 'Tape sur la photo pour agrandir • Appui long pour changer'
                : 'Tape sur l\'avatar pour ajouter une photo',
  ...
  // ──────────────────────────────────────────────
  //  ONGLET 2 — TEMPS D'ÉCRAN
  // ──────────────────────────────────────────────
  Widget _buildScreenTimeTab(dynamic child, FamilyProvider provider) {
    final saturday = provider.getScreenTime(widget.childId, 'saturday');
    final sunday = provider.getScreenTime(widget.childId, 'sunday');
    final satBonus =
        provider.getScreenTimeBonus(widget.childId, 'saturday');
    final sunBonus =
        provider.getScreenTimeBonus(widget.childId, 'sunday');
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        children: [
            ...
            children: [
              child.hasPhoto
                  ...
                  : CircleAvatar(
                      radius: 24,
                      backgroundColor:
                          _getLevelColor(child.points).withOpacity(0.2),
                      child: Text(
                        child.name.isNotEmpty
                            ? child.name[0].toUpperCase()
                            : '?',
              ...
              Expanded(
                child: Column(
                  ...
                  children: [
                    ...
                    const Text('Temps d\'écran du week-end',
                        style: TextStyle(
          ...
          // Cartes samedi / dimanche
          Row(
            children: [
              Expanded(
                child: _screenTimeCard(
                    'Samedi', saturday, satBonus, Colors.cyan),
              ),
              const SizedBox(width: 12),
              Expanded(
                child: _screenTimeCard(
                    'Dimanche', sunday, sunBonus, Colors.purple),
              ),
            ],
          ),
          const SizedBox(height: 16),
            ...
            child: Padding(
              ...
              child: Row(
                ...
                children: [
                  ...
                  Column(
                    children: [
                      const Text('Total week-end',
                          style: TextStyle(
                              fontWeight: FontWeight.bold, fontSize: 16)),
                      Text(
                        _formatMinutes(
                            saturday + sunday + satBonus + sunBonus),
                        style: const TextStyle(
                            fontSize: 22,
                            fontWeight: FontWeight.bold,
          ...
          if (context.read<PinProvider>().canPerformParentAction()) ...[
            GlassCard(
              child: Padding(
                padding: const EdgeInsets.all(16),
                child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    const Text('⏰ Ajuster le temps bonus',
                        style: TextStyle(fontWeight: FontWeight.bold)),
                    const SizedBox(height: 12),
                    Row(
                      children: [
                        Expanded(
                          child:
                              _bonusButton('Samedi', 'saturday', provider),
                        ),
                        const SizedBox(width: 8),
                        Expanded(
                          child: _bonusButton(
                              'Dimanche', 'sunday', provider),
                        ),
                      ],
                    ),
                  ],
  ...
  Widget _screenTimeCard(
      String day, int minutes, int bonus, Color color) {
    final total = minutes + bonus;
      ...
      child: Padding(
        ...
        child: Column(
          children: [
              ...
              child: Stack(
                children: [
                  ...
                  Center(
                    child: Text(
                      hours > 0
                          ? '${hours}h${mins > 0 ? mins.toString().padLeft(2, '0') : ''}'
                          : '${mins}m',
                      style: const TextStyle(
            ...
            const SizedBox(height: 8),
            Text(
              '${_formatMinutes(minutes)} base',
              style: const TextStyle(fontSize: 11, color: Colors.white38),
            ),
            if (bonus > 0)
              Text(
                '+${_formatMinutes(bonus)} bonus',
                style: TextStyle(
  ...
  Widget _bonusButton(
      String label, String day, FamilyProvider provider) {
    return TvFocusWrapper(
      onTap: () => _showBonusDialog(day, provider),
      child: GlassCard(
        onTap: () => _showBonusDialog(day, provider),
        child: Padding(
          ...
          child: Column(
            children: [
              const Icon(Icons.add_alarm, color: Colors.orangeAccent),
              const SizedBox(height: 4),
  ...
  void _showBonusDialog(String day, FamilyProvider provider) {
    int bonusMinutes = 15;
    showDialog(
      context: context,
      builder: (ctx) => StatefulBuilder(
        builder: (ctx, setDialogState) => AlertDialog(
          ...
          title: Text(
              '⏰ Bonus — ${day == 'saturday' ? 'Samedi' : 'Dimanche'}'),
          content: Column(
            mainAxisSize: MainAxisSize.min,
            children: [
              Row(
                ...
                children: [
                  IconButton(
                    onPressed: bonusMinutes > 5
                        ? () =>
                            setDialogState(() => bonusMinutes -= 5)
                        : null,
                    icon: const Icon(Icons.remove_circle_outline),
                  ),
                  Text('$bonusMinutes min',
                      style: const TextStyle(
              ...
              Slider(
                value: bonusMinutes.toDouble(),
                min: 5,
                max: 120,
                divisions: 23,
                activeColor: Colors.orangeAccent,
                label: '$bonusMinutes min',
                onChanged: (v) =>
                    setDialogState(() => bonusMinutes = v.round()),
              ),
            ],
          ),
          actions: [
            ...
            ElevatedButton(
              onPressed: () {
                ...
                ScaffoldMessenger.of(context).showSnackBar(
                  SnackBar(
                    content: Text(
                        '⏰ +$bonusMinutes min ajoutées pour ${day == 'saturday' ? 'samedi' : 'dimanche'}'),
                    backgroundColor: Colors.green.withOpacity(0.8),
                    behavior: SnackBarBehavior.floating,
                    shape: RoundedRectangleBorder(
                        borderRadius: BorderRadius.circular(12)),
              ...
              child: Text('Ajouter $bonusMinutes min'),
            ),
          ],
        ),
      ),
  ...
  Widget _buildHistoryTab(FamilyProvider provider) {
    ...
    return ListView.builder(
      ...
      itemBuilder: (_, index) {
        ...
        final displayReason = entry.reason.contains('|')
                ...
                trailing: entry.hasProofPhoto
                    ? GestureDetector(
                        onTap: () => _showFullPhoto(
                            context, entry.proofPhotoBase64!),
                        child: ClipRRect(
  ...
  Widget _buildBadgesTab(dynamic child, FamilyProvider provider) {
    final allBadges = provider.getAllBadges();
    ...
    return GridView.builder(
      ...
      itemBuilder: (_, index) {
        ...
        final progress = badge.requiredPoints > 0
        ...
        return TvFocusWrapper(
          ...
          child: GlassCard(
            ...
            onTap: () =>
                _showBadgeDetail(context, badge, isUnlocked, child),
            child: Padding(
              padding: const EdgeInsets.all(8),
              child: Column(
                mainAxisAlignment: MainAxisAlignment.center,
        ...
        child: Column(
          ...
          children: [
            ...
            if (isUnlocked)
              ...
              const SizedBox(height: 8),
              Text(
                '${child.points} / ${badge.requiredPoints} points',
                style: const TextStyle(
                    fontWeight: FontWeight.bold, fontSize: 16),
              ),
              const SizedBox(height: 4),
              Text(
                'Encore ${(badge.requiredPoints - child.points).clamp(0, 999999)} points nécessaires',
                style: const TextStyle(
                    color: Colors.white54, fontSize: 12),
              ),
            ],
