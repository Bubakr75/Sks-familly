...
class _RotatingTrophy extends StatefulWidget {
  ...
  final bool unlocked;
  final double size;
  const _RotatingTrophy(
      {required this.emoji, required this.unlocked, this.size = 32});
  @override
  State<_RotatingTrophy> createState() => _RotatingTrophyState();
}

class _RotatingTrophyState extends State<_RotatingTrophy>
  ...
  Widget build(BuildContext context) {
    ...
    return AnimatedBuilder(
      ...
      builder: (context, _) {
        final t = _ctrl.value;
        // scaleX min 0.05 pour éviter un rendu "miroir" négatif
        final scaleX = cos(t * 2 * pi).abs().clamp(0.05, 1.0);
...
class _BadgeUnlockedAnimation extends StatefulWidget {
  ...
  const _BadgeUnlockedAnimation(
      {required this.emoji, required this.name, required this.onComplete});
  @override
  State<_BadgeUnlockedAnimation> createState() =>
      _BadgeUnlockedAnimationState();
}

class _BadgeUnlockedAnimationState extends State<_BadgeUnlockedAnimation>
        ...
        return Stack(alignment: Alignment.center, children: [
          ...
          CustomPaint(
              ...
              painter: _SparkPainter(
                  _sparks, _sparkCtrl.value, _mainCtrl.value)),
          // Halo doré
          Container(
            width: 140,
            height: 140,
          ...
          // Texte "POUVOIR DÉBLOQUÉ"
          Positioned(
            bottom: MediaQuery.of(context).size.height * 0.3,
            child: FadeTransition(
              opacity: _textFade,
              child: Column(mainAxisSize: MainAxisSize.min, children: [
                const Text('POUVOIR DÉBLOQUÉ !',
                    style: TextStyle(
                        color: Colors.amber,
                        fontSize: 22,
...
class BadgesScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      floatingActionButton: FloatingActionButton(
        onPressed: () =>
            PinGuard.guardAction(context, () => _showAddBadgeDialog(context)),
                    ...
                    child: Row(children: [
                      ...
                      TvFocusWrapper(
                        ...
                        child: IconButton(
                            ...
                            onPressed: () =>
                                _showBadgeInfo(context, allBadges)),
                      ),
                    ]),
                  ),
                  const SizedBox(height: 8),
                            ...
                            children: [
                              ...
                              GridView.builder(
                                ...
                                itemBuilder: (context, index) {
                                  ...
                                  final anyUnlocked = children.any(
                                      (c) => c.points >= badge.requiredPoints);
                                  return TvFocusWrapper(
                                    onTap: badge.isCustom
                                        ? () => PinGuard.guardAction(
                                            context,
                                            () => _showEditBadgeDialog(
                                                context, provider, badge))
                                        : null,
                              ...
                              const Text('Progression par enfant',
                                  style: TextStyle(
                                      fontSize: 16,
                                      fontWeight: FontWeight.bold,
                                      color: Colors.white70)),
                              ...
                              ...children.map((child) {
                                final earned = allBadges
                                    .where((b) =>
                                        child.points >= b.requiredPoints)
                                    .toList();
                                final progress = allBadges.isEmpty
                                    ? 0.0
                                    : earned.length / allBadges.length;
                                final nextBadges = allBadges
                                    .where((b) =>
                                        child.points < b.requiredPoints)
                                    .toList()
                                  ..sort((a, b) => a.requiredPoints
                                      .compareTo(b.requiredPoints));
                                final nextBadge = nextBadges.isNotEmpty
                                    ? nextBadges.first
                                    : null;
    ...
    showDialog(
      ...
      builder: (ctx) => AlertDialog(
        ...
        title: Row(children: [
          ...
          const SizedBox(width: 8),
          const Text('Système de pouvoirs',
        ...
        content: SizedBox(
          ...
          child: ListView(shrinkWrap: true, children: [
            const Text(
                'Les enfants débloquent des pouvoirs en accumulant des points. Chaque pouvoir donne un privilège spécial !',
            ...
            ...badges.map((b) => Padding(
                  ...
                  child: Row(children: [
                    _RotatingTrophy(
                        emoji: b.powerEmoji, unlocked: true, size: 20),
  ...
  void _showAddBadgeDialog(BuildContext context) {
    ...
    // Callback unique pour éviter la duplication onTap/onPressed
    void submitBadge() {
      final name = nameCtrl.text.trim();
      final pts = int.tryParse(pointsCtrl.text) ?? 50;
      if (name.isEmpty || pts < 1) return;
      context.read<FamilyProvider>().addCustomBadge(
            name,
            selectedPower,
            descCtrl.text.trim(),
            pts,
      ...
      ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Pouvoir "$name" créé !')));
    }
          ...
          content: SingleChildScrollView(
            ...
            Wrap(
                ...
                children: powerOptions.map((opt) {
                  ...
                  return TvFocusWrapper(
                    onTap: () =>
                        setState(() => selectedPower = opt['type'] as String),
                    child: GestureDetector(
                      onTap: () => setState(
                          () => selectedPower = opt['type'] as String),
  ...
  void _showEditBadgeDialog(
      BuildContext context, FamilyProvider provider, BadgeModel badge) {
    showDialog(
      context: context,
      builder: (ctx) => AlertDialog(
        backgroundColor: const Color(0xFF1A1A2E),
        ...
        title: Row(children: [
          _RotatingTrophy(
              emoji: badge.powerEmoji, unlocked: true, size: 22),
          const SizedBox(width: 8),
          Expanded(
          ...
          TextButton(
            onPressed: () {
              ...
              ScaffoldMessenger.of(context).showSnackBar(SnackBar(
                  content: Text('Pouvoir "${badge.name}" supprimé')));
            },
