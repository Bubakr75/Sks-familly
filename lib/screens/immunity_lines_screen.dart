...
class _ImmunityLinesScreenState extends State<ImmunityLinesScreen> {
              ...
              return Container(
                ...
                child: ListView(
                  ...
                  children: [
                    ...
                    const Text('Nouvelle immunité', style: TextStyle(color: Colors.white, fontSize: 20, fontWeight: FontWeight.bold), textAlign: TextAlign.center),
                    ...
                    TextField(
                      ...
                      decoration: InputDecoration(hintText: 'Ex: Excellent bulletin...', hintStyle: const TextStyle(color: Colors.white38), filled: true, fillColor: Colors.white.withOpacity(0.06),
                    ...
                    Row(mainAxisAlignment: MainAxisAlignment.center, children: [
                      TvFocusWrapper(onTap: () { if (lines > 1) setModalState(() => lines--); },
                      ...
                      Text('$lines', style: const TextStyle(color: Colors.cyanAccent, fontSize: 36, fontWeight: FontWeight.bold)),
                      const SizedBox(width: 24),
                      TvFocusWrapper(onTap: () { if (lines < 20) setModalState(() => lines++); },
                    ...
                    const SizedBox(height: 20),
                    TvFocusWrapper(onTap: () => setModalState(() => hasExpiry = !hasExpiry),
                      child: Row(children: [
                    ...
                    SizedBox(width: double.infinity, height: 52, child: TvFocusWrapper(
                      onTap: () {
                        if (selectedChildId == null || reason.isEmpty) { ScaffoldMessenger.of(ctx).showSnackBar(const SnackBar(content: Text('Sélectionnez un enfant et une raison'), backgroundColor: Colors.orangeAccent)); return; }
                        provider.addImmunity(selectedChildId!, reason, lines, expiresAt: hasExpiry ? DateTime.now().add(Duration(days: expiryDays)) : null);
                        Navigator.pop(ctx);
                        ScaffoldMessenger.of(this.context).showSnackBar(SnackBar(content: Text('$lines ligne(s) d\'immunité ajoutée(s)'), backgroundColor: Colors.green));
                      },
  ...
  Widget build(BuildContext context) {
    return AnimatedBackground(
      child: Scaffold(
        ...
        appBar: AppBar(title: const Text('Lignes d\'immunité'), backgroundColor: Colors.transparent, elevation: 0),
                        ...
                        child: GlassCard(
                          child: Padding(
                            ...
                            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                              Row(children: [
                                CircleAvatar(radius: 20, backgroundColor: Colors.cyanAccent.withOpacity(0.3),
                                ...
                                Container(padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 6),
                                  decoration: BoxDecoration(color: Colors.greenAccent.withOpacity(0.15), borderRadius: BorderRadius.circular(20), border: Border.all(color: Colors.greenAccent.withOpacity(0.5))),
                                  child: Text('$totalLines lignes dispo', style: const TextStyle(color: Colors.greenAccent, fontSize: 12, fontWeight: FontWeight.bold))),
                              ]),
    ...
    return Padding(
      ...
      child: TvFocusWrapper(
        ...
        child: Container(
          ...
          decoration: BoxDecoration(color: statusColor.withOpacity(0.06), borderRadius: BorderRadius.circular(12), border: Border.all(color: statusColor.withOpacity(0.2))),
          child: Row(children: [
            ...
            Container(padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 4), decoration: BoxDecoration(color: statusColor.withOpacity(0.15), borderRadius: BorderRadius.circular(8)),
  ...
  void _showImmunityDetail(dynamic immunity, dynamic child) {
    ...
    showModalBottomSheet(
      ...
      builder: (ctx) => DraggableScrollableSheet(
        initialChildSize: 0.55, minChildSize: 0.3, maxChildSize: 0.8,
        builder: (context, scrollController) => Container(
          decoration: BoxDecoration(color: Colors.grey[900]?.withOpacity(0.95), borderRadius: const BorderRadius.vertical(top: Radius.circular(24))),
          child: ListView(controller: scrollController, padding: const EdgeInsets.all(20), children: [
            Center(child: Container(width: 40, height: 4, decoration: BoxDecoration(color: Colors.white24, borderRadius: BorderRadius.circular(2)))),
            const SizedBox(height: 16),
            Row(children: [Icon(_getStatusIcon(status), color: statusColor, size: 28), const SizedBox(width: 12), Expanded(child: Text(immunity.reason, style: const TextStyle(color: Colors.white, fontSize: 18, fontWeight: FontWeight.bold)))]),
            const SizedBox(height: 20),
            _detailRow('Enfant', child.name), _detailRow('Statut', _getStatusLabel(status), valueColor: statusColor),
            _detailRow('Lignes totales', '${immunity.lines}'), _detailRow('Lignes utilisées', '${immunity.usedLines}'), _detailRow('Lignes restantes', '${immunity.availableLines}'),
            _detailRow('Créée le', _formatDate(immunity.createdAt)),
            if (immunity.expiresAt != null) _detailRow('Expire le', _formatDate(immunity.expiresAt!), valueColor: status == 'expired' ? Colors.redAccent : Colors.orangeAccent),
            const SizedBox(height: 24),
            if (status == 'usable')
              Row(children: [
                Expanded(child: TvFocusWrapper(onTap: () { Navigator.pop(ctx); _confirmDelete(immunity); },
                  child: OutlinedButton.icon(onPressed: () { Navigator.pop(ctx); _confirmDelete(immunity); }, icon: const Icon(Icons.delete_outline, size: 18), label: const Text('Supprimer'),
                ...
                const SizedBox(width: 12),
                Expanded(child: TvFocusWrapper(onTap: () { Navigator.pop(ctx); _tradeImmunity(immunity, child); },
                  child: ElevatedButton.icon(onPressed: () { Navigator.pop(ctx); _tradeImmunity(immunity, child); }, icon: const Icon(Icons.swap_horiz, size: 18), label: const Text('Échanger'),
  ...
  void _confirmDelete(dynamic immunity) {
    showDialog(context: context, builder: (ctx) => AlertDialog(
      backgroundColor: Colors.grey[900], title: const Text('Supprimer l\'immunité ?', style: TextStyle(color: Colors.white)),
      ...
      actions: [
        ...
        TvFocusWrapper(onTap: () { context.read<FamilyProvider>().removeImmunity(immunity.id); Navigator.pop(ctx); setState(() {}); },
          child: ElevatedButton(onPressed: () { context.read<FamilyProvider>().removeImmunity(immunity.id); Navigator.pop(ctx); setState(() {}); },
            style: ElevatedButton.styleFrom(backgroundColor: Colors.redAccent), child: const Text('Supprimer'))),
      ],
    ));
  }
