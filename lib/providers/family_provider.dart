...
class FamilyProvider extends ChangeNotifier {
  ...
  late Box _tradesBox;
  late Box _schoolNotesBox; // ✅ NOUVEAU
  ...
  Future<void> init() async {
    ...
    _schoolNotesBox = await Hive.openBox('school_notes'); // ✅ NOUVEAU
    _loadLocal();
    try {
  ...
  Future<String> addPoints(
    ...
    notifyListeners();
    return entry.id; // ✅ RETOURNE L'ID pour annulation
  }
  ...
  int getTotalAvailableImmunity(String childId) =>
      _immunities.where((im) => im.childId == childId && im.isUsable)
  ...
  List<TradeModel> getActiveTrades() =>
      _trades.where((t) => t.isActive).toList();
  ...
  List<HistoryEntry> getRecentHistory(String childId, {int limit = 20}) =>
      _history.where((h) => h.childId == childId).take(limit).toList();
  ...
  // ─── Helpers pour historique (utilisés par addHistoryEntry) ──
  void addHistoryEntry({
    ...
    _history.insert(0, entry);
    _historyBox.put(entry.id, jsonEncode(entry.toMap()));
    if (_firestore.isConnected) _firestore.saveHistoryEntry(entry);
    notifyListeners();
  }

  // ─── Getters compatibles (aliases) ──────────────────────────
  List<PunishmentLines> getPunishments(String childId) =>
  ...
  void submitPunishmentLines({
    ...
    try {
      final p = _punishments.firstWhere((p) => p.id == punishmentId);
      // On stocke la soumission en attente dans les métadonnées
      // Note: Cela nécessite que PunishmentLines ait un champ pour ça
      // Pour l'instant on utilise la box directement
      final data = jsonDecode(_punishmentsBox.get(p.id) as String) as Map<String, dynamic>;
      data['pendingSubmission'] = {
        'linesSubmitted': linesSubmitted,
        'proofPhotoBase64': proofPhotoBase64,
        'submittedAt': DateTime.now().toIso8601String(),
      };
      data['lastRejection'] = null;
      _punishmentsBox.put(p.id, jsonEncode(data));
      notifyListeners();
    } catch (_) {}
  }

  Map<String, dynamic>? getPunishmentRawData(String punishmentId) {
    try {
  ...
  void validatePunishmentSubmission({
    ...
    try {
      final p = _punishments.firstWhere((pp) => pp.id == punishmentId);
      p.completedLines = (p.completedLines + linesValidated).clamp(0, p.totalLines);

      final data = jsonDecode(_punishmentsBox.get(p.id) as String) as Map<String, dynamic>;
      data['completedLines'] = p.completedLines;
      data['pendingSubmission'] = null;
      final validations = List<Map<String, dynamic>>.from(data['validationHistory'] ?? []);
      validations.add({
        'linesValidated': linesValidated,
        'parentNote': parentNote,
        'validatedAt': DateTime.now().toIso8601String(),
      });
      data['validationHistory'] = validations;
      _punishmentsBox.put(p.id, jsonEncode(data));
      if (_firestore.isConnected) _firestore.savePunishment(p);
  ...
  void rejectImmunityRequest({
    ...
    try {
      final p = _punishments.firstWhere((pp) => pp.id == punishmentId);
      final data = jsonDecode(_punishmentsBox.get(p.id) as String) as Map<String, dynamic>;
      data['pendingImmunityRequest'] = null;
      _punishmentsBox.put(p.id, jsonEncode(data));
      notifyListeners();
    } catch (_) {}
  }
  ...
  void validateSchoolNote({
    ...
    addPoints(
      childId,
      bonusPoints,
      '$subject|$adjustedValue|$maxValue',
  ...
  Future<void> deleteHistoryEntry({
    ...
    if (reversePoints && entry.points != 0) {
      ...
      if (child != null) {
        ...
        child.points = child.points.clamp(0, 999999);
        child.level = child.currentLevelNumber;
        await _childrenBox.put(child.id, jsonEncode(child.toMap()));
        if (_firestore.isConnected) await _firestore.saveChild(child);
      }
  ...
  Future<void> editHistoryEntry({
    ...
    if (child != null) {
      ...
      if (newPoints >= 0) {
        ...
        child.points += newPoints; // négatif
      }
      child.points = child.points.clamp(0, 999999);
      child.level = child.currentLevelNumber;
      await _childrenBox.put(child.id, jsonEncode(child.toMap()));
    ...
    // Recréer l'entrée avec les nouvelles valeurs
    final updated = HistoryEntry(
      ...
      isBonus:            newPoints >= 0,
      proofPhotoBase64:   entry.proofPhotoBase64,
      date:               entry.date,
      actionBy:           entry.actionBy,
    );
    _history[index] = updated;
    await _historyBox.put(updated.id, jsonEncode(updated.toMap()));
    if (_firestore.isConnected) await _firestore.saveHistoryEntry(updated);
    notifyListeners();
  }

  Future<void> clearChildHistory({required String childId}) async {
    final toRemove = _history.where((h) => h.childId == childId).toList();
    for (final entry in toRemove) {
      _history.remove(entry);
      await _historyBox.delete(entry.id);
      if (_firestore.isConnected) await _firestore.deleteHistoryEntry(entry.id);
    }
    notifyListeners();
  }

  Future<void> clearAllPunishments({required String childId}) async {
    final toRemove = _punishments.where((p) => p.childId == childId).toList();
    for (final p in toRemove) {
      await removePunishment(p.id);
    }
  }

  Future<void> clearAllImmunities({required String childId}) async {
    final toRemove = _immunities.where((im) => im.childId == childId).toList();
    for (final im in toRemove) {
      await removeImmunity(im.id);
    }
  }

  Future<void> resetChildPoints({required String childId}) async {
    final child = getChild(childId);
    if (child == null) return;
    child.points = 0;
    child.level = child.currentLevelNumber;
  ...
  Future<void> reactivateImmunity({
    required String childId,
    required String immunityId,
  }) async {
    try {
  ...
  Future<void> resetPunishmentProgress({
    required String childId,
    required String punishmentId,
  }) async {
    try {
  ...
  Future<void> completePunishment({
    required String childId,
    required String punishmentId,
  }) async {
    try {
  ...
  Future<void> resetEverything() async {
    for (final child in _children) {
      await clearChildHistory(childId: child.id);
      await clearAllPunishments(childId: child.id);
      await clearAllImmunities(childId: child.id);
      child.points = 0;
      child.level = 1;
  ...
  void updateChildPoints(String childId, int newPoints) {
    final child = getChild(childId);
    if (child == null) return;
    child.points = newPoints.clamp(0, 999999);
    child.level = child.currentLevelNumber;
...
} // ← FIN DE LA CLASSE FamilyProvider
