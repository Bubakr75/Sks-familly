...
class FamilyProvider extends ChangeNotifier {
  ...
  List<NoteModel> get notes =>
      _notesBox.values.map((m) => NoteModel.fromMap(Map<String, dynamic>.from(m))).toList();
  ...
  List<TribunalCase> get tribunalCases =>
      _tribunalBox.values.map((m) => TribunalCase.fromMap(Map<String, dynamic>.from(m))).toList();
  ...
  Future<void> updateChildPhoto(String childId, String photoBase64) async {
    final child = getChild(childId);
    if (child == null) return;
    final updated = child.copyWith(photoBase64: photoBase64.isEmpty ? null : photoBase64);
    await _childrenBox.put(childId, updated.toMap());
    notifyListeners();
    _syncToFirestore();
  }
  ...
  Future<void> addPoints(String childId, int points, String description,
    ...
    final entry = HistoryEntry(
      id: entryId, childId: childId, points: points.abs(), reason: description,
      category: category ?? (isBonus ? 'Bonus' : 'Malus'),
      date: DateTime.now(), isBonus: isBonus,
      proofPhotoBase64: proofPhotoBase64, actionBy: actionBy,
    );
  ...
  List<HistoryEntry> getChildBonuses(String childId) =>
      getHistoryForChild(childId).where((e) => e.isBonus).toList();
  ...
  Future<void> deleteHistoryEntry(String id) async {
    ...
    if (raw != null) {
      ...
      if (child != null) {
        final adjustment = entry.isBonus ? -entry.points : entry.points;
        final updated = child.copyWith(points: child.points + adjustment);
  ...
  Future<void> editHistoryEntry(String id, {int? points, String? reason}) async {
    ...
    if (points != null) {
      ...
      final child = getChild(childId);
      if (child != null) {
        final oldActual = wasBonus ? oldPoints : -oldPoints;
        final newActual = wasBonus ? points : -points;
        final diff = newActual - oldActual;
  ...
  Future<void> addGoal(String childId, String title, int targetPoints) async {
    final id = DateTime.now().millisecondsSinceEpoch.toString();
    await _goalsBox.put(id, {'id': id, 'childId': childId, 'title': title,
      'targetPoints': targetPoints, 'isCompleted': false, 'createdAt': DateTime.now().toIso8601String()});
    notifyListeners(); _syncToFirestore();
  }
  ...
  Future<void> addPunishment(String childId, String reason, int totalLines, {String? phrase}) async {
    final id = DateTime.now().millisecondsSinceEpoch.toString();
    await _punishmentsBox.put(id, {
      'id': id, 'childId': childId, 'reason': reason, 'totalLines': totalLines,
      'completedLines': 0, 'phrase': phrase ?? reason, 'status': 'active',
      'submissions': <Map<String, dynamic>>[], 'createdAt': DateTime.now().toIso8601String(),
    });
    notifyListeners(); _syncToFirestore();
  }
  ...
  Future<void> updatePunishment(String id, Map<String, dynamic> data) async {
    ...
    p.addAll(data);
    await _punishmentsBox.put(id, p);
    notifyListeners(); _syncToFirestore();
  }
  ...
  Future<void> submitPunishmentLines(String punishmentId, int lines, {String? photoBase64}) async {
    ...
    if (raw == null) return;
    final p = Map<String, dynamic>.from(raw);
    final subs = List<Map<String, dynamic>>.from(p['submissions'] ?? []);
    subs.add({'lines': lines, 'photoBase64': photoBase64,
      'submittedAt': DateTime.now().toIso8601String(), 'status': 'pending'});
    p['submissions'] = subs;
    await _punishmentsBox.put(punishmentId, p);
    notifyListeners(); _syncToFirestore();
  }

  Future<void> validatePunishmentSubmission(String punishmentId, int submissionIndex,
      {bool accepted = true, bool approved = true, int? validatedLines, String? note}) async {
    final raw = _punishmentsBox.get(punishmentId);
    if (raw == null) return;
    final p = Map<String, dynamic>.from(raw);
    final subs = List<Map<String, dynamic>>.from(p['submissions'] ?? []);
    if (submissionIndex >= subs.length) return;
    final sub = Map<String, dynamic>.from(subs[submissionIndex]);
    final isApproved = accepted && approved;
    if (isApproved) {
      final lines = validatedLines ?? (sub['lines'] as int? ?? 0);
      sub['status'] = 'validated'; sub['validatedLines'] = lines; sub['note'] = note;
      p['completedLines'] = (p['completedLines'] as int? ?? 0) + lines;
      if ((p['completedLines'] as int) >= (p['totalLines'] as int)) {
        p['status'] = 'completed'; p['completedAt'] = DateTime.now().toIso8601String();
      }
    } else {
      sub['status'] = 'rejected'; sub['note'] = note;
    }
    subs[submissionIndex] = sub;
    p['submissions'] = subs;
    await _punishmentsBox.put(punishmentId, p);
    notifyListeners(); _syncToFirestore();
  }
  ...
  Future<void> resetPunishmentProgress(String id) async {
    ...
    if (raw == null) return;
    final p = Map<String, dynamic>.from(raw);
    p['completedLines'] = 0; p['status'] = 'active'; p['submissions'] = <Map<String, dynamic>>[];
    await _punishmentsBox.put(id, p);
    notifyListeners(); _syncToFirestore();
  }
  ...
  Future<void> completePunishment(String id) async {
    ...
    if (raw == null) return;
    final p = Map<String, dynamic>.from(raw);
    p['status'] = 'completed'; p['completedLines'] = p['totalLines'];
  ...
  Future<void> addImmunity(String childId, String reason, int lines, {DateTime? expiresAt}) async {
    final id = DateTime.now().millisecondsSinceEpoch.toString();
    final imm = ImmunityLines(id: id, childId: childId, reason: reason,
      lines: lines, usedLines: 0, createdAt: DateTime.now(), expiresAt: expiresAt);
  ...
  Future<void> useImmunityOnPunishment(String immunityId, String punishmentId) async {
    ...
    final toUse = remaining < imm.availableLines ? remaining : imm.availableLines;
    ...
    if ((pun['completedLines'] as int) >= (pun['totalLines'] as int)) {
      pun['status'] = 'completed'; pun['completedAt'] = DateTime.now().toIso8601String();
    }
    await _punishmentsBox.put(punishmentId, pun);
    notifyListeners(); _syncToFirestore();
  }

  Future<void> useImmunityOnPunishmentNamed({required String immunityId, required String punishmentId}) async {
  ...
  Future<void> removeImmunity(String id) async => deleteImmunity(id);
  ...
  Future<void> addCustomBadge(String name, String icon, String description,
    ...
    final badge = BadgeModel(id: id, name: name, icon: icon, description: description,
  ...
  int getSaturdayMinutes(String childId) {
    final st = getScreenTime(childId);
    return (st?['saturdayMinutes'] as int?) ?? 0;
  }

  int getSundayMinutes(String childId) {
    final st = getScreenTime(childId);
    return (st?['sundayMinutes'] as int?) ?? 0;
  }

  int getParentBonusMinutes(String childId) {
    final st = getScreenTime(childId);
    return (st?['bonusMinutes'] as int?) ?? 0;
  }
  ...
  int getWeeklyScore(String childId) {
    ...
    return history.where((e) => e.childId == childId && e.date.isAfter(start))
        .fold<int>(0, (sum, e) => sum + (e.isBonus ? e.points : -e.points));
  }

  // ─── Tribunal ─────────────────────────────────────────
  Future<void> fileTribunalCase({
    required String title, required String description,
    required String plaintiffId, required String accusedId,
  }) async {
    final id = DateTime.now().millisecondsSinceEpoch.toString();
    final tc = TribunalCase(id: id, title: title, description: description,
      plaintiffId: plaintiffId, accusedId: accusedId, status: TribunalStatus.filed);
  ...
  Future<void> renderVerdict({
    required String caseId, required TribunalVerdict verdict,
    String? reason, int accusedPoints = 0,
  }) async {
    final raw = _tribunalBox.get(caseId);
    if (raw == null) return;
    final tc = TribunalCase.fromMap(Map<String, dynamic>.from(raw));
    ...
    await _tribunalBox.put(caseId, tc.toMap());
    if (accusedPoints != 0) {
      await addPoints(tc.accusedId, accusedPoints, 'Verdict tribunal: ${reason ?? verdict.name}',
          category: 'Tribunal');
    }
    notifyListeners(); _syncToFirestore();
  }

  Future<void> voteTribunal(String caseId, String voterId, TribunalVerdict vote) async {
    final raw = _tribunalBox.get(caseId);
    if (raw == null) return;
    final tc = TribunalCase.fromMap(Map<String, dynamic>.from(raw));
  ...
  Future<void> createTrade(String fromChildId, String toChildId, int lines, String serviceDescription) async {
    final id = DateTime.now().millisecondsSinceEpoch.toString();
    final trade = TradeModel(id: id, fromChildId: fromChildId, toChildId: toChildId,
      immunityLines: lines, serviceDescription: serviceDescription, status: 'pending');
    await _tradesBox.put(id, trade.toMap());
    notifyListeners(); _syncToFirestore();
  }

  Future<void> acceptTrade(String id) async {
    await _updateTradeStatus(id, 'accepted');
  }

  Future<void> rejectTrade(String id) async {
    await _updateTradeStatus(id, 'rejected');
  }

  Future<void> cancelTrade(String id) async {
    await _updateTradeStatus(id, 'cancelled');
  }

  Future<void> markServiceDone(String id) async {
    await _updateTradeStatus(id, 'service_done');
  }
  ...
  Future<void> completeTrade(String id, {String? parentNote}) async {
    ...
    // Transfer immunity lines
    final trade = TradeModel.fromMap(m);
    await addImmunity(trade.toChildId, 'Achat: ${trade.serviceDescription}', trade.immunityLines);
  ...
  List<TradeModel> getTradesForChild(String childId) {
    return trades.where((t) => t.fromChildId == childId || t.toChildId == childId).toList();
  }

  List<TradeModel> getPendingTradesForChild(String childId) {
    return trades.where((t) => t.isPending && t.toChildId == childId).toList();
  }
  ...
  Future<void> clearHistory([String? childId]) async {
    if (childId == null) { await _historyBox.clear(); }
    else {
      final keys = <dynamic>[];
      for (final key in _historyBox.keys) {
        final m = Map<String, dynamic>.from(_historyBox.get(key)!);
  ...
  Future<void> clearAllPunishments({String? childId}) async {
    if (childId == null) { await _punishmentsBox.clear(); }
    else {
      final keys = <dynamic>[];
  ...
  Future<void> clearAllImmunities({String? childId}) async {
    if (childId == null) { await _immunitiesBox.clear(); }
    else {
      final keys = <dynamic>[];
  ...
  Future<void> resetEverything() async {
    await _childrenBox.clear(); await _historyBox.clear(); await _goalsBox.clear();
    await _notesBox.clear(); await _punishmentsBox.clear(); await _immunitiesBox.clear();
    await _tribunalBox.clear(); await _customBadgesBox.clear(); await _screenTimeBox.clear();
    await _tradesBox.clear(); await _schoolNotesBox.clear();
    _familyCode = null; _isSyncEnabled = false; await _metaBox.clear();
    notifyListeners();
  }
  ...
  Future<void> addSchoolNote(String childId, String subject, double grade,
    ...
    final id = DateTime.now().millisecondsSinceEpoch.toString();
    await _schoolNotesBox.put(id, {'id': id, 'childId': childId, 'subject': subject,
      'grade': grade, 'maxGrade': maxGrade, 'comment': comment, 'photoBase64': photoBase64,
      'isValidated': false, 'createdAt': DateTime.now().toIso8601String()});
    notifyListeners(); _syncToFirestore();
  }

  Future<void> validateSchoolNote(String noteId, {bool validated = true, int bonusPoints = 0, String? childId}) async {
    ...
    if (validated && bonusPoints > 0 && childId != null) {
      await addPoints(childId, bonusPoints, 'Bonus note scolaire: ${note['subject']}', category: 'École');
    }
    notifyListeners(); _syncToFirestore();
  }
