...
class FamilyProvider extends ChangeNotifier {
  ...
  Future<void> removeChild(String id) async {
    ...
    await _saveAllLocal();
    if (_firestore.isConnected) {
      await _firestore.deleteChild(id);
    }
    notifyListeners();
  }

  // === POINTS (MODIFIED FOR PROOF PHOTO) ===
  ...
  Future<void> resetAllScores() async {
    ...
    if (_firestore.isConnected) {
      await _firestore.resetAllScores();
    }
    notifyListeners();
  }
  ...
  Future<void> toggleGoal(String goalId) async {
    ...
    if (idx != -1) {
      ...
      if (_firestore.isConnected) {
        await _firestore.saveGoal(_goals[idx]);
      }
      notifyListeners();
    }
  }
  ...
  Future<void> removeGoal(String goalId) async {
    ...
    if (_firestore.isConnected) {
      await _firestore.deleteGoal(goalId);
    }
    notifyListeners();
  }
  ...
  Future<void> incrementPunishmentLines(String pId) async {
    ...
    if (idx != -1 && !_punishments[idx].isCompleted) {
      ...
      if (_firestore.isConnected) {
        await _firestore.savePunishment(_punishments[idx]);
      }
      notifyListeners();
    }
  }
  ...
  Future<void> removePunishment(String pId) async {
    ...
    if (_firestore.isConnected) {
      await _firestore.deletePunishment(pId);
    }
    notifyListeners();
  }
  ...
  Future<void> addNote(String childId, String text, {String authorName = 'Parent'}) async {
    ...
    if (_firestore.isConnected) {
      await _firestore.saveNote(note);
    }
    notifyListeners();
  }
  ...
  Future<void> updateNote(String noteId, String newText) async {
    ...
    if (idx != -1) {
      ...
      if (_firestore.isConnected) {
        await _firestore.saveNote(_notes[idx]);
      }
      notifyListeners();
    }
  }
  ...
  Future<void> removeNote(String noteId) async {
    ...
    if (_firestore.isConnected) {
      await _firestore.deleteNote(noteId);
    }
    notifyListeners();
  }
